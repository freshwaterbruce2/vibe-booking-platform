...
export class RefundService {
  ...
  static calculateRefund(
    ...
    const hoursUntilCheckIn = Math.floor((checkInDate.getTime() - cancellationDate.getTime()) / (1000 * 60 * 60));
  ...
  static async processAutomaticRefund(refundRequest: RefundRequest) {
    try {
      ...
      const refundCalculation = this.calculateRefund(bookingData);
      ...
      const refundAmount = refundRequest.requestedAmount && 
      ...
      await this.sendRefundNotifications(bookingData, stripeRefund, refundCalculation);
  ...
  static async cancelBookingWithRefund(
    ...
    try {
      ...
      const shouldProcessAutomatically = 
        ...
        refundCalculation.finalRefundAmount === refundCalculation.refundableAmount; // Full or standard refund

      if (shouldProcessAutomatically) {
        // Process automatic refund
        return await this.processAutomaticRefund({
          bookingId,
        ...
        // Create manual refund request for admin review
        return await this.createManualRefundRequest({
          bookingId,
          reason: cancellationReason,
          requestedBy,
          notes,
  ...
  static async getRefundHistory(bookingId: string) {
    try {
      const refundList = await db.select()
        .from(refunds)
