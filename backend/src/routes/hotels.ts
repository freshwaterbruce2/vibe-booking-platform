...
const searchHotelsSchema = z.object({
  ...
  starRating: z.string().optional().transform((val) => val ? val.split(',').map(Number) : undefined),
  amenities: z.string().optional().transform((val) => val ? val.split(',') : undefined),
  passions: z.string().optional().transform((val) => val ? val.split(',') : undefined),
...
const calculatePassionScore = (hotel: any, passions: string[]): number => {
  ...
  const passionKeywords: Record<string, string[]> = {
    ...
    'outdoor-adventure': ['hiking', 'adventure', 'outdoor', 'nature', 'mountain', 'trail', 'sports', 'activity'],
    'wellness-spa': ['spa', 'wellness', 'massage', 'relaxation', 'therapy', 'health', 'meditation', 'yoga'],
    'cultural-explorer': ['museum', 'culture', 'historic', 'art', 'heritage', 'local', 'traditional', 'tour'],
    'luxury-indulgence': ['luxury', 'premium', 'suite', 'concierge', 'exclusive', 'high-end', 'deluxe', 'vip'],
    'business-networking': ['business', 'conference', 'meeting', 'wifi', 'workspace', 'center', 'professional'],
    'romantic-getaway': ['romantic', 'couple', 'honeymoon', 'private', 'intimate', 'sunset', 'special', 'romance'],
  };
...
hotelsRouter.post('/search', validateRequest(searchHotelsSchema), async (req, res) => {
  try {
    ...
    if (params.query && !params.destination) {
      try {
        ...
        logger.warn('AI parsing failed, using original params', { error: aiError });
      }
    }
    ...
    if (searchParams.passions && searchParams.passions.length > 0) {
      ...
      if (searchParams.sortBy === 'popularity') {
        processedHotels.sort((a, b) => (b.passionScore || 0) - (a.passionScore || 0));
      }
    }
...
hotelsRouter.get('/:hotelId', validateRequest(hotelDetailsSchema, 'params'), async (req, res) => {
  try {
    ...
    logger.error('Failed to get hotel details', { error, hotelId: req.params.hotelId });
    res.status(500).json({
      error: 'Fetch Error',
...
hotelsRouter.get('/:hotelId/availability', validateRequest(availabilitySchema, 'query'), async (req, res) => {
  try {
    const { hotelId } = req.params;
    ...
    logger.error('Failed to check availability', { error, params: req.params, query: req.query });
    res.status(500).json({
...
hotelsRouter.get('/:hotelId/rooms/:roomId/rates', validateRequest(roomRatesSchema, 'query'), async (req, res) => {
  try {
    ...
    logger.error('Failed to get room rates', { error, params: req.params, query: req.query });
    res.status(500).json({
      error: 'Rates Error',
...
hotelsRouter.get('/search/suggestions', async (req, res) => {
  try {
    ...
    logger.error('Failed to get search suggestions', { error, query: req.query });
    res.json({
      success: true,
      data: {
        suggestions: [],
...
hotelsRouter.post('/compare', async (req, res) => {
  try {
    ...
    if (!Array.isArray(hotelIds) || hotelIds.length === 0 || hotelIds.length > 5) {
      return res.status(400).json({
        error: 'Validation Error',
