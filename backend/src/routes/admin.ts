 ...
 */
adminRouter.get('/dashboard', validateRequest(dashboardMetricsSchema, 'query'), async (req: Request, res: Response) => {
  try {
    ...
    const metrics = await CommissionService.getDashboardMetrics(start, end, currency as string);
 ...
 */
adminRouter.get('/revenue-report', validateRequest(revenueReportSchema, 'query'), async (req: Request, res: Response) => {
  try {
...
adminRouter.get('/bookings', async (req: Request, res: Response) => {
  try {
    ...
    if (paymentStatus) {
      conditions = and(conditions, eq(bookings.paymentStatus, paymentStatus as string));
    }
    
    if (startDate) {
      conditions = and(conditions, gte(bookings.createdAt, new Date(startDate as string)));
    }
    
    if (endDate) {
      conditions = and(conditions, lte(bookings.createdAt, new Date(endDate as string)));
    }
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          totalRevenue: parseFloat(summaryStats[0]?.totalRevenue?.toString() || '0'),
          averageOrderValue: parseFloat(summaryStats[0]?.averageOrderValue?.toString() || '0'),
          totalBookings: parseInt(summaryStats[0]?.totalBookings?.toString() || '0'),
        },
      },
    });
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          totalAmount: parseFloat(summaryStats[0]?.totalAmount?.toString() || '0'),
          averageAmount: parseFloat(summaryStats[0]?.averageAmount?.toString() || '0'),
          totalPayments: parseInt(summaryStats[0]?.totalPayments?.toString() || '0'),
        },
      },
    });
 ...
 */
adminRouter.post('/refund/approve', validateRequest(refundApprovalSchema), async (req: Request, res: Response) => {
  try {
 ...
 */
adminRouter.get('/analytics/top-metrics', async (req: Request, res: Response) => {
  try {
    ...
    const topMetrics = {
      topHotels: [
        { hotelId: 'hotel_1', name: 'Grand Hotel', bookings: 45, revenue: 12500 },
        { hotelId: 'hotel_2', name: 'Ocean View Resort', bookings: 38, revenue: 11200 },
        { hotelId: 'hotel_3', name: 'City Center Hotel', bookings: 32, revenue: 9800 },
      ],
      topCountries: [
 ...
 */
adminRouter.get('/export/revenue-report', async (req: Request, res: Response) => {
  try {
    ...
    const reportData = await CommissionService.getDashboardMetrics(start, end);
 ...
 */
function generateRevenueCSV(reportData: any): string {
  const headers = [
    'Metric',
    'Current Period',
    'Previous Period',
  ...
  const rows = [
    ['Total Revenue', reportData.current.totalRevenue, reportData.previous.totalRevenue, reportData.growth.revenue],
    ['Total Bookings', reportData.current.totalBookings, reportData.previous.totalBookings, reportData.growth.bookings],
    ['Total Commissions', reportData.current.totalCommissions, reportData.previous.totalCommissions, reportData.growth.commissions],
    ['Average Order Value', reportData.current.averageOrderValue, reportData.previous.averageOrderValue, ''],
    ['Payment Success Rate', reportData.current.paymentSuccessRate, reportData.previous.paymentSuccessRate, ''],
  ];
