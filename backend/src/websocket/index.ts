  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('subscribe:payments', (bookingId: string) => {
      if (user?.id && bookingId) {
        ...
        logger.debug('User subscribed to payment updates', { userId: user.id, bookingId });
      }
    });
    ...
    socket.on('join:search', (sessionId: string) => {
      if (sessionId && user?.id) {
        ...
        logger.debug('User joined search session', { userId: user.id, sessionId });
      }
    });

    socket.on('search:update', (data: { sessionId: string; searchParams: any }) => {
...
export const emitBookingUpdate = (io: SocketIOServer, userId: string, bookingData: any) => {
  io.to(`bookings:${userId}`).emit('booking:updated', bookingData);
};

export const emitPaymentUpdate = (io: SocketIOServer, bookingId: string, paymentData: any) => {
