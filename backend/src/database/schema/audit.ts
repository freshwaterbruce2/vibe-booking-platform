import { pgTable, text, timestamp, varchar, jsonb, uuid, index, integer, boolean, numeric } from 'drizzle-orm/pg-core';
...
export const auditLog = pgTable('audit_log', {
  ...
  return {
    ...
    impersonatedByIdx: index('audit_log_impersonated_by_idx').on(table.impersonatedBy),
    ...
    // Complex queries
    userTimeIdx: index('audit_log_user_time_idx').on(table.changedBy, table.changedAt),
    tableTimeIdx: index('audit_log_table_time_idx').on(table.tableName, table.changedAt),
  };
});
...
export const userActivityLog = pgTable('user_activity_log', {
  ...
  return {
    ...
    resourceIdIdx: index('user_activity_log_resource_id_idx').on(table.resourceId),
    
    // Success/error analysis
    successIdx: index('user_activity_log_success_idx').on(table.success),
    errorCodeIdx: index('user_activity_log_error_code_idx').on(table.errorCode),
    
    // Time-based analysis
    occurredAtIdx: index('user_activity_log_occurred_at_idx').on(table.occurredAt),
    
    // Security monitoring
    ipAddressIdx: index('user_activity_log_ip_address_idx').on(table.ipAddress),
    geoCountryIdx: index('user_activity_log_geo_country_idx').on(table.geoCountry),
    
    // Performance monitoring
    responseTimeIdx: index('user_activity_log_response_time_idx').on(table.responseTimeMs),
    
    // Complex queries
    userActionIdx: index('user_activity_log_user_action_idx').on(table.userId, table.action),
    userTimeIdx: index('user_activity_log_user_time_idx').on(table.userId, table.occurredAt),
    actionTimeIdx: index('user_activity_log_action_time_idx').on(table.action, table.occurredAt),
  };
});
 ...
 */
export const securityEventsLog = pgTable('security_events_log', {
  id: uuid('id').defaultRandom().primaryKey(),
  ...
  return {
    // Event classification
    eventTypeIdx: index('security_events_log_event_type_idx').on(table.eventType),
    ...
    // Security monitoring
    ipAddressIdx: index('security_events_log_ip_address_idx').on(table.ipAddress),
    
    // Detection analysis
    detectionMethodIdx: index('security_events_log_detection_method_idx').on(table.detectionMethod),
    ruleIdIdx: index('security_events_log_rule_id_idx').on(table.ruleId),
    
    // Response tracking
    actionTakenIdx: index('security_events_log_action_taken_idx').on(table.actionTaken),
    resolvedByIdx: index('security_events_log_resolved_by_idx').on(table.resolvedBy),
    
    // Time-based analysis
    occurredAtIdx: index('security_events_log_occurred_at_idx').on(table.occurredAt),
    resolvedAtIdx: index('security_events_log_resolved_at_idx').on(table.resolvedAt),
    
    // Complex queries
    severityTimeIdx: index('security_events_log_severity_time_idx').on(table.severity, table.occurredAt),
    ipEventIdx: index('security_events_log_ip_event_idx').on(table.ipAddress, table.eventType),
    unresolvedIdx: index('security_events_log_unresolved_idx').on(table.severity) 
...
export const performanceLog = pgTable('performance_log', {
  ...
  return {
    // Metric tracking
    metricTypeIdx: index('performance_log_metric_type_idx').on(table.metricType),
    operationIdx: index('performance_log_operation_idx').on(table.operation),
    
    // Performance analysis
    responseTimeIdx: index('performance_log_response_time_idx').on(table.responseTimeMs),
    memoryUsageIdx: index('performance_log_memory_usage_idx').on(table.memoryUsageMb),
    cpuUsageIdx: index('performance_log_cpu_usage_idx').on(table.cpuUsagePercent),
    
    // Database performance
    dbQueryCountIdx: index('performance_log_db_query_count_idx').on(table.dbQueryCount),
    dbQueryTimeIdx: index('performance_log_db_query_time_idx').on(table.dbQueryTimeMs),
    cacheHitIdx: index('performance_log_cache_hit_idx').on(table.cacheHit),
    
    // Error tracking
    errorOccurredIdx: index('performance_log_error_occurred_idx').on(table.errorOccurred),
    
    // Time-based analysis
    recordedAtIdx: index('performance_log_recorded_at_idx').on(table.recordedAt),
    
    // Complex queries
    operationTimeIdx: index('performance_log_operation_time_idx').on(table.operation, table.recordedAt),
    performanceIdx: index('performance_log_performance_idx').on(table.metricType, table.responseTimeMs),
    slowQueriesIdx: index('performance_log_slow_queries_idx').on(table.responseTimeMs) 
...
export type NewSecurityEventsLog = z.infer<typeof insertSecurityEventsLogSchema>;
