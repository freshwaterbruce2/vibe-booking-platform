import { pgTable, text, timestamp, varchar, numeric, jsonb, uuid, index, integer, boolean, real, unique, check } from 'drizzle-orm/pg-core';
...
export const hotels = pgTable('hotels', {
  ...
  // Meta
  checkInTime: varchar('check_in_time', { length: 10 }).notNull().default('15:00'),
  ...
  return {
    ...
    cityCountryIdx: index('hotels_city_country_idx').on(table.city, table.country), // Composite for location searches
    ...
    activeFeaturedIdx: index('hotels_active_featured_idx').on(table.isActive, table.isFeatured),
    
    // Location indexes
    locationIdx: index('hotels_location_idx').on(table.latitude, table.longitude),
    neighborhoodIdx: index('hotels_neighborhood_idx').on(table.neighborhood),
    
    // Complex search indexes
    searchIdx: index('hotels_search_idx').on(table.city, table.isActive, table.rating),
    priceSearchIdx: index('hotels_price_search_idx').on(table.city, table.priceMin, table.priceMax, table.isActive),
    
    // Full-text search index
    searchVectorIdx: index('hotels_search_vector_idx').using('gin', sql`to_tsvector('english', coalesce(${table.name}, '') || ' ' || coalesce(${table.description}, '') || ' ' || coalesce(${table.city}, ''))`),
    
    // Unique constraints
    slugUnique: unique('hotels_slug_unique').on(table.slug),
    externalIdUnique: unique('hotels_external_id_unique').on(table.externalId),
    
    // Data validation constraints
    ratingCheck: check('hotels_rating_check', sql`${table.rating} >= 0 AND ${table.rating} <= 5`),
    starRatingCheck: check('hotels_star_rating_check', sql`${table.starRating} >= 1 AND ${table.starRating} <= 5`),
    priceCheck: check('hotels_price_check', sql`${table.priceMin} >= 0 AND ${table.priceMax} >= ${table.priceMin}`),
    coordinatesCheck: check('hotels_coordinates_check', sql`${table.latitude} BETWEEN -90 AND 90 AND ${table.longitude} BETWEEN -180 AND 180`),
  };
});
...
export const roomAvailability = pgTable('room_availability', {
  ...
  return {
    roomDateIdx: index('room_availability_room_date_idx').on(table.roomId, table.date),
