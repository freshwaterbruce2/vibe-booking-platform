 ...
 */
export const hotelPerformanceView = pgView('hotel_performance_view').as((qb) => {
  return qb
    .select({
      ...
      totalBookings: sql`COUNT(CASE WHEN b.created_at >= NOW() - INTERVAL '30 days' THEN 1 END)`.as('total_bookings_30d'),
      confirmedBookings: sql`COUNT(CASE WHEN b.status = 'confirmed' AND b.created_at >= NOW() - INTERVAL '30 days' THEN 1 END)`.as('confirmed_bookings_30d'),
      cancelledBookings: sql`COUNT(CASE WHEN b.status = 'cancelled' AND b.created_at >= NOW() - INTERVAL '30 days' THEN 1 END)`.as('cancelled_bookings_30d'),
      
      // Revenue metrics (last 30 days)
      totalRevenue: sql`COALESCE(SUM(CASE WHEN b.status = 'confirmed' AND b.created_at >= NOW() - INTERVAL '30 days' THEN b.total_amount END), 0)`.as('total_revenue_30d'),
      averageBookingValue: sql`COALESCE(AVG(CASE WHEN b.status = 'confirmed' AND b.created_at >= NOW() - INTERVAL '30 days' THEN b.total_amount END), 0)`.as('avg_booking_value_30d'),
      
      // Occupancy metrics
      averageStayLength: sql`COALESCE(AVG(CASE WHEN b.status = 'confirmed' AND b.created_at >= NOW() - INTERVAL '30 days' THEN b.nights END), 0)`.as('avg_stay_length_30d'),
      occupancyRate: sql`COALESCE((COUNT(CASE WHEN b.status = 'confirmed' AND b.created_at >= NOW() - INTERVAL '30 days' THEN 1 END) * 100.0) / NULLIF(h.total_rooms * 30, 0), 0)`.as('occupancy_rate_30d'),
      
      // Customer metrics
      averageRating: sql`h.rating`.as('average_rating'),
      totalReviews: sql`h.review_count`.as('total_reviews'),
      repeatCustomers: sql`COUNT(DISTINCT CASE WHEN customer_bookings.booking_count > 1 THEN b.user_id END)`.as('repeat_customers'),
    ...
    .groupBy(sql`h.id, h.name, h.city, h.country, h.rating, h.review_count, h.total_rooms`);
});

/**
 * Daily booking metrics for time-series analysis
 */
export const dailyBookingMetricsView = pgView('daily_booking_metrics_view').as((qb) => {
  return qb
    .select({
      ...
      confirmedBookings: sql`COUNT(CASE WHEN b.status = 'confirmed' THEN 1 END)`.as('confirmed_bookings'),
      cancelledBookings: sql`COUNT(CASE WHEN b.status = 'cancelled' THEN 1 END)`.as('cancelled_bookings'),
      pendingBookings: sql`COUNT(CASE WHEN b.status = 'pending' THEN 1 END)`.as('pending_bookings'),
      
      // Revenue metrics
      totalRevenue: sql`COALESCE(SUM(CASE WHEN b.status = 'confirmed' THEN b.total_amount END), 0)`.as('total_revenue'),
      averageBookingValue: sql`COALESCE(AVG(CASE WHEN b.status = 'confirmed' THEN b.total_amount END), 0)`.as('avg_booking_value'),
      ...
      // Customer metrics
      uniqueCustomers: sql`COUNT(DISTINCT COALESCE(b.user_id, b.guest_email))`.as('unique_customers'),
      repeatCustomers: sql`COUNT(DISTINCT CASE WHEN repeat_customers.booking_count > 1 THEN COALESCE(b.user_id, b.guest_email) END)`.as('repeat_customers'),
 ...
 */
export const searchPerformanceView = pgView('search_performance_view').as((qb) => {
  return qb
    .select({
      ...
      uniqueUsers: sql`COUNT(DISTINCT COALESCE(sa.user_id, sa.session_id))`.as('unique_users'),
      
      // Search types
      naturalLanguageSearches: sql`COUNT(CASE WHEN sa.query_type = 'natural_language' THEN 1 END)`.as('natural_language_searches'),
      filteredSearches: sql`COUNT(CASE WHEN sa.query_type = 'filtered' THEN 1 END)`.as('filtered_searches'),
      mapBasedSearches: sql`COUNT(CASE WHEN sa.query_type = 'map_based' THEN 1 END)`.as('map_based_searches'),
      
      // Performance metrics
      averageExecutionTime: sql`AVG(sa.execution_time_ms)`.as('avg_execution_time_ms'),
      cacheHitRate: sql`(COUNT(CASE WHEN sa.cache_hit THEN 1 END) * 100.0) / COUNT(*)`.as('cache_hit_rate_percent'),
      
      // Result metrics
      averageResultCount: sql`AVG(sa.total_results)`.as('avg_result_count'),
      clickThroughRate: sql`CASE WHEN SUM(sa.total_results) > 0 THEN (SUM(sa.clicked_results) * 100.0) / SUM(sa.total_results) ELSE 0 END`.as('click_through_rate_percent'),
      conversionRate: sql`CASE WHEN SUM(sa.clicked_results) > 0 THEN (SUM(sa.booked_results) * 100.0) / SUM(sa.clicked_results) ELSE 0 END`.as('conversion_rate_percent'),
 ...
 */
export const revenueAnalyticsView = pgView('revenue_analytics_view').as((qb) => {
  return qb
    .select({
      country: sql`h.country`.as('country'),
      city: sql`h.city`.as('city'),
      ...
      // Revenue metrics
      totalRevenue: sql`SUM(CASE WHEN b.status = 'confirmed' THEN b.total_amount ELSE 0 END)`.as('total_revenue'),
      totalBookings: sql`COUNT(CASE WHEN b.status = 'confirmed' THEN 1 END)`.as('total_bookings'),
      averageBookingValue: sql`AVG(CASE WHEN b.status = 'confirmed' THEN b.total_amount END)`.as('avg_booking_value'),
      
      // Guest metrics
      totalGuests: sql`SUM(CASE WHEN b.status = 'confirmed' THEN b.adults + b.children ELSE 0 END)`.as('total_guests'),
      averagePartySize: sql`AVG(CASE WHEN b.status = 'confirmed' THEN b.adults + b.children END)`.as('avg_party_size'),
      
      // Occupancy metrics
      totalNights: sql`SUM(CASE WHEN b.status = 'confirmed' THEN b.nights ELSE 0 END)`.as('total_nights'),
      averageStayLength: sql`AVG(CASE WHEN b.status = 'confirmed' THEN b.nights END)`.as('avg_stay_length'),
      
      // Performance metrics
      cancellationRate: sql`(COUNT(CASE WHEN b.status = 'cancelled' THEN 1 END) * 100.0) / COUNT(*)`.as('cancellation_rate_percent'),
      revenuePerNight: sql`SUM(CASE WHEN b.status = 'confirmed' THEN b.total_amount END) / NULLIF(SUM(CASE WHEN b.status = 'confirmed' THEN b.nights END), 0)`.as('revenue_per_night'),
