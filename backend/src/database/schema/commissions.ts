...
export const commissions = pgTable('commissions', {
  ...
  commissionRate: numeric('commission_rate', { precision: 5, scale: 4 }).notNull().default('0.0500'), // 5% default
  ...
  hotelEarnings: numeric('hotel_earnings', { precision: 10, scale: 2 }).notNull(),
...
export const revenueReports = pgTable('revenue_reports', {
  ...
  // Revenue metrics
  totalBookings: numeric('total_bookings', { precision: 10, scale: 0 }).notNull().default('0'),
  totalRevenue: numeric('total_revenue', { precision: 12, scale: 2 }).notNull().default('0.00'),
  totalCommissions: numeric('total_commissions', { precision: 12, scale: 2 }).notNull().default('0.00'),
  ...
  // Booking metrics
  successfulBookings: numeric('successful_bookings', { precision: 10, scale: 0 }).notNull().default('0'),
  cancelledBookings: numeric('cancelled_bookings', { precision: 10, scale: 0 }).notNull().default('0'),
  refundedBookings: numeric('refunded_bookings', { precision: 10, scale: 0 }).notNull().default('0'),
  averageOrderValue: numeric('average_order_value', { precision: 10, scale: 2 }).notNull().default('0.00'),
  
  // Payment metrics
  totalPayments: numeric('total_payments', { precision: 10, scale: 0 }).notNull().default('0'),
  successfulPayments: numeric('successful_payments', { precision: 10, scale: 0 }).notNull().default('0'),
  failedPayments: numeric('failed_payments', { precision: 10, scale: 0 }).notNull().default('0'),
  paymentSuccessRate: numeric('payment_success_rate', { precision: 5, scale: 2 }).notNull().default('0.00'),
  ...
  return {
    reportTypeIdx: index('revenue_reports_report_type_idx').on(table.reportType),
    reportDateIdx: index('revenue_reports_report_date_idx').on(table.reportDate),
...
export const payoutBatches = pgTable('payout_batches', {
  ...
  commissionCount: numeric('commission_count', { precision: 10, scale: 0 }).notNull(),
  currency: varchar('currency', { length: 3 }).notNull().default('USD'),
  ...
  return {
    ...
    processedAtIdx: index('payout_batches_processed_at_idx').on(table.processedAt),
  };
});
