import { initializeSqliteDatabase, getSqliteConnection, getSqliteDb, closeSqliteDatabase } from './sqlite';
...
async function setupSqliteDatabase() {
  try {
    ...
    // Test the database
    const result = sqlite.prepare('SELECT COUNT(*) as count FROM sqlite_master WHERE type=?').get('table') as { count: number };
