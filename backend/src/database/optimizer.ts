...
export class HotelSearchOptimizer {
  ...
  async searchHotels(
    ...
    searchQuery?: string
  ): Promise<{ hotels: HotelSearchResult[]; metrics: SearchPerformanceMetrics }> {
  ...
  async fullTextSearch(
    ...
    const query = this.db
      ...
      .orderBy(desc(sql`ts_rank_cd(to_tsvector('english', ${hotels.name} || ' ' || ${hotels.description} || ' ' || ${hotels.city}), plainto_tsquery('english', ${searchQuery}))`))
  ...
  async getHotelAvailability(
    ...
    logger.info(`Hotel availability query executed in ${Date.now() - startTime}ms`);
   ...
   * Cache management
   */
  private generateCacheKey(filters: SearchFilters, searchQuery?: string): string {
   ...
   */
  private determineQueryComplexity(filters: SearchFilters, searchQuery?: string): 'simple' | 'moderate' | 'complex' {
    let complexity = 0;
    
    if (searchQuery) complexity += 2;
    if (filters.amenities?.length) complexity += 2;
    if (filters.checkIn && filters.checkOut) complexity += 3;
    ...
    for (const search of frequentSearches.rows) {
      try {
        ...
        if (filters.city && filters.priceRange) {
          recommendations.push('CREATE INDEX CONCURRENTLY hotels_city_price_range_idx ON hotels(city, price_range) WHERE is_active = true;');
        }
        if (filters.amenities) {
          recommendations.push('CREATE INDEX CONCURRENTLY hotels_amenities_gin_idx ON hotels USING gin(amenities);');
        }
      } catch (error) {
        // Skip invalid JSON
      }
