...
const dbPath = process.env.DATABASE_URL || path.join(process.cwd(), 'database', 'vibe-booking.db');
...
const rawSqliteConfig = {
  ...
  cors: {
    origin: process.env.CORS_ORIGIN?.split(',') || ['http://localhost:5173', 'http://localhost:5174', 'http://localhost:3000', 'http://localhost:3009'],
  },
  database: {
    path: dbPath,
