...
export class AuditLogger {
  ...
  private static getEventTypeFromRequest(req: Request): AuditEventType {
    if (req.path.includes('/auth/login')) return AuditEventType.USER_LOGIN;
    if (req.path.includes('/auth/logout')) return AuditEventType.USER_LOGOUT;
    if (req.path.includes('/auth/register')) return AuditEventType.USER_REGISTER;
    if (req.path.includes('/payment')) return AuditEventType.PAYMENT_CREATED;
    if (req.path.includes('/admin')) return AuditEventType.ADMIN_ACCESS;
    return AuditEventType.SENSITIVE_DATA_ACCESS;
  }

  private static assessRequestRisk(req: Request, res: Response, duration: number): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
    // Critical: Authentication failures, payment failures
    if (res.statusCode === 401 || res.statusCode === 403) return 'CRITICAL';
    if (req.path.includes('/payment') && res.statusCode >= 400) return 'CRITICAL';
    ...
    if (duration > 10000) return 'HIGH'; // > 10 seconds

    // Medium: User operations, client errors
    if (req.path.includes('/auth') || req.path.includes('/user')) return 'MEDIUM';
    if (res.statusCode >= 400) return 'MEDIUM';

    return 'LOW';
  }

  private static assessQueryRisk(query: string): 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' {
    const upperQuery = query.toUpperCase();
    
    // Critical: Direct SQL modifications without proper params
    if (upperQuery.includes('DROP ') || upperQuery.includes('TRUNCATE ')) return 'CRITICAL';
    if (upperQuery.includes('ALTER ') && upperQuery.includes('SCHEMA')) return 'CRITICAL';

    // High: Data modifications
    if (upperQuery.includes('DELETE ') || upperQuery.includes('UPDATE ')) return 'HIGH';
    if (upperQuery.includes('INSERT ') && upperQuery.includes('users')) return 'HIGH';
  ...
  private static sanitizeRequestBody(body: any, path: string): any {
    ...
    const sensitiveFields = ['password', 'token', 'secret', 'key', 'cvv', 'cardNumber'];
 ...
 */
export const auditAuthEvent = (eventType: AuditEventType, riskLevel: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' = 'MEDIUM') => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    await AuditLogger.logEvent({
      ...
      action: req.method,
      details: {
        requestBody: AuditLogger.prototype.constructor.prototype.sanitizeRequestBody(req.body, req.path),
      },
      risk_level: riskLevel,
      timestamp: new Date(),
    });
