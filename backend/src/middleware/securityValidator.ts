...
export class SecurityConfigValidator {
  ...
  static validateSecurityConfig(): void {
    ...
    issues.push(...this.validateJWTSecrets());
    
    // Validate CORS configuration
    issues.push(...this.validateCORSConfig());
    
    // Validate encryption settings
    issues.push(...this.validateEncryptionConfig());
    
    // Validate environment-specific settings
    issues.push(...this.validateEnvironmentConfig());
    ...
    if (issues.length > 0) {
      ...
      if (isProduction) {
        ...
        throw new Error(`Production deployment blocked due to security configuration issues: ${issues.join(', ')}`);
      } else {
  ...
  private static validateJWTSecrets(): string[] {
    ...
    if (this.isWeakSecret(refreshSecret)) {
      issues.push(`JWT refresh secret is weak or using default value`);
    }
    
    // Check reset token secret
    const resetSecret = config.jwt.resetSecret;
    if (this.isWeakSecret(resetSecret)) {
      issues.push(`JWT reset secret is weak or using default value`);
    }
    
    // Ensure secrets are different
    if (jwtSecret === refreshSecret || jwtSecret === resetSecret || refreshSecret === resetSecret) {
      issues.push('JWT secrets must be unique - using same secret for multiple purposes');
    }
  ...
  private static validateEncryptionConfig(): string[] {
    ...
    if (!config.stripe.webhookSecret || config.stripe.webhookSecret.length < 10) {
  ...
  private static isWeakSecret(secret: string): boolean {
    ...
    if (entropy < this.MINIMUM_SECRET_ENTROPY) {
      return true;
    }
 ...
 */
export const runtimeSecurityChecks = (req: Request, res: Response, next: NextFunction) => {
  ...
  const suspiciousPattern = suspiciousPatterns.find(pattern => pattern.test(userInput));
 ...
 */
export const validateApiKey = (keyName: string, required: boolean = true) => {
