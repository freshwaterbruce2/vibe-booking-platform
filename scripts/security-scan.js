...
async function scanDependencies() {
  ...
  try {
    ...
    if (vulnerabilities.critical > 0 || vulnerabilities.high > 0) {
      ...
      return { passed: false, vulnerabilities: vulnerabilities.critical + vulnerabilities.high };
    }
...
async function checkSecurityHeaders() {
  ...
  try {
    ...
    const serverPath = path.join(process.cwd(), 'build-website-example/server/server.js');
...
async function runOWASPChecks() {
  ...
  const checks = {
    'SQL Injection': async () => {
      ...
      const { stdout } = await execAsync('grep -r "SELECT\\|INSERT\\|UPDATE\\|DELETE" src/ || true');
      return stdout.trim().length === 0;
    },
    'XSS': async () => {
      // Check for dangerouslySetInnerHTML
      const { stdout } = await execAsync('grep -r "dangerouslySetInnerHTML" src/ || true');
      return stdout.trim().length === 0;
    },
    'CSRF': async () => {
