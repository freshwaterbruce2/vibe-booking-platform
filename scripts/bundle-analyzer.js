...
async function analyzeBundleSizes() {
  ...
  try {
    ...
    const { stdout } = await execAsync(`find ${assetsPath} -name "*.js" -o -name "*.css" | sort`);
    ...
    if (totalKB > THRESHOLDS.total) {
      issues.push(`Total bundle size (${totalKB.toFixed(2)} KB) exceeds threshold (${THRESHOLDS.total} KB)`);
    }
    
    // Check for large individual bundles
    [...bundles.js, ...bundles.css].forEach(file => {
      if (file.sizeKB > THRESHOLDS.mainBundle) {
        issues.push(`${file.name} (${file.sizeKB.toFixed(2)} KB) is larger than recommended`);
      }
    });
...
async function compareBundleSizes(currentBundles) {
  ...
  try {
    ...
    log.info(`Change: ${difference > 0 ? '+' : ''}${difference.toFixed(2)} KB (${percentChange}%)`);
    
    // Check thresholds
    const issues = [];
    
    if (percentChange > THRESHOLDS.increase.error) {
      issues.push(`Bundle size increased by ${percentChange}% (threshold: ${THRESHOLDS.increase.error}%)`);
    } else if (percentChange > THRESHOLDS.increase.warning) {
      log.warning(`Bundle size increased by ${percentChange}% (warning threshold: ${THRESHOLDS.increase.warning}%)`);
    }
...
async function analyzeCodeSplitting() {
  ...
  try {
    const distPath = path.join(process.cwd(), 'dist');
    const { stdout } = await execAsync(`find ${distPath} -name "*.js" | grep -E "(chunk|vendor)" | wc -l`);
    ...
    const { stdout: lazyRoutes } = await execAsync(`grep -r "lazy(" ${srcPath} | wc -l`);
...
async function analyzeDependencies() {
  ...
  try {
    ...
    const { stdout: duplicates } = await execAsync('npm ls --depth=0 | grep "deduped" | wc -l');
    const duplicateCount = parseInt(duplicates.trim());
    
    if (duplicateCount > 0) {
      log.warning(`Found ${duplicateCount} duplicate dependencies that could be deduped`);
    ...
    for (const dep of dependencies) {
      const { stdout } = await execAsync(`grep -r "from ['"]${dep}" src/ || true`);
      if (!stdout.trim()) {
        unusedDeps.push(dep);
      }
    }
