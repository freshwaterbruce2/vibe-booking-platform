      ...
      if (await searchButton.isVisible()) {
        ...
        for (let i = 0; i < Math.min(buttons.length, 10); i++) {
          ...
          if (text.toLowerCase().includes('book') || 
            ...
            await page.screenshot({ path: 'screenshots/direct-03-after-booking-click.png', fullPage: true });
            break;
          }
        }
        ...
        if (paymentFound) {
          ...
          await page.screenshot({ path: 'screenshots/direct-05-payment-found.png', fullPage: true });
        } else {
          console.log('❌ No payment integration found yet');
          
          // Try to navigate further
          console.log('📍 Looking for next step buttons...');
          const nextButtons = page.locator('button:has-text("Continue"), button:has-text("Next"), button:has-text("Proceed")');
