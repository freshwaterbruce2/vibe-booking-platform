...
test.describe('Complete Booking Flow - Cross-Device Journey', () => {
  ...
  test.describe('Search and Discovery Flow', () => {
    ...
    test('should handle AI-powered search flow', async ({ page }) => {
      ...
      // Use AI search input
      const aiInput = page.locator('input[placeholder*="Romantic weekend in Paris with spa"]');
      ...
      const hasResults = await page.locator('h3:has-text("hotels found")').count() > 0;
  ...
  test.describe('Hotel Selection and Details', () => {
    ...
    test('should display hotel details with conversion elements', async ({ page }) => {
      ...
      if (await hotelCards.count() > 0) {
        ...
        const hotelName = firstCard.locator('.text-lg.sm\\:text-xl.font-semibold');
    ...
    test('should handle hotel card interactions', async ({ page }) => {
      ...
      if (await hotelCards.count() > 0) {
        ...
        const hasExpandedContent = await page.locator('.expanded, .detailed').count() > 0;
        
        // One of these should be true
        expect(currentUrl.includes('/hotel/') || hasModal || hasExpandedContent).toBe(true);
      }
    });

    test('should show additional hotel information on demand', async ({ page }) => {
      const hotelCards = page.locator('.group.hover\\:shadow-2xl');
      ...
      if (await hotelCards.count() > 0) {
        ...
        // Check for amenities
        const amenities = firstCard.locator('.inline-flex.items-center.gap-1.px-2.py-1');
  ...
  test.describe('Booking Initiation Flow', () => {
    ...
    test('should initiate booking flow from Book Now button', async ({ page }) => {
      ...
      if (await bookNowButtons.count() > 0) {
        ...
        const hotelName = await hotelCard.locator('.text-lg.sm\\:text-xl.font-semibold').textContent();
        ...
        const currentUrl = page.url();
        const hasBookingPage = currentUrl.includes('/booking') || currentUrl.includes('/payment');
        const hasBookingModal = await page.locator('[role="dialog"]:has-text("booking"), .booking-modal').count() > 0;
        const hasBookingForm = await page.locator('form:has(input[type="email"]), .booking-form').count() > 0;
    ...
    test('should handle booking flow on mobile devices', async ({ page }) => {
      ...
      if (await bookNowButtons.count() > 0) {
        ...
        const bookingElements = await page.locator('form, [role="dialog"], .booking').count();
    ...
    test('should preserve search context in booking flow', async ({ page }) => {
      ...
      if (await bookNowButtons.count() > 0) {
        ...
        const hasDestination = bodyText?.toLowerCase().includes(destination.toLowerCase());
  ...
  test.describe('Guest Information and Forms', () => {
        ...
        if (await emailInput.count() > 0 || await nameInput.count() > 0) {
          ...
          if (await emailInput.count() > 0) {
            ...
            if (await continueButton.count() > 0) {
              ...
              const hasError = await page.locator('.error, [role="alert"], .invalid').count() > 0;
              if (hasError) {
    ...
    test('should validate required booking information', async ({ page }) => {
      ...
      try {
        ...
        // Test form validation
        const submitButton = page.locator('button[type="submit"], button:has-text("Book"), button:has-text("Continue")');
        
        if (await submitButton.count() > 0) {
          await submitButton.click();
          
          // Should show validation errors for empty required fields
          const errorElements = page.locator('.error, [role="alert"], .invalid, .required');
  ...
  test.describe('Payment Integration Flow', () => {
    test('should display payment options', async ({ page }) => {
      ...
      try {
        ...
        const squarePaymentForm = page.locator('#card-container, .square-payment, [data-testid="square-payment"]');
        ...
        const paypalOption = page.locator('button:has-text("PayPal"), .paypal-button');
        ...
        const securityIndicators = page.locator('text=Secure, text=SSL, text=256-bit');
        ...
        if (await bookNowButtons.count() > 0) {
          ...
          const paymentElements = page.locator('#card-container, .square-payment, input[placeholder*="card"], .payment-form');
    ...
    test('should handle payment form validation', async ({ page }) => {
      ...
      try {
        ...
        const paymentSubmit = page.locator('button:has-text("Pay"), button:has-text("Complete"), button[type="submit"]');
  ...
  test.describe('Booking Confirmation Flow', () => {
    test('should display confirmation page structure', async ({ page }) => {
      ...
      try {
        ...
        const confirmationHeading = page.locator('h1:has-text("Confirmation"), h2:has-text("Confirmed"), h1:has-text("Thank you")');
        ...
        const nextSteps = page.locator('text=email, text=contact, text=support');
  ...
  test.describe('Error Handling in Booking Flow', () => {
    test('should handle search errors gracefully', async ({ page }) => {
      ...
      const errorMessage = page.locator('text=error, text=Error, [role="alert"]');
      const noResultsMessage = page.locator('text=No hotels found');
      
      const hasErrorHandling = await errorMessage.count() > 0 || await noResultsMessage.count() > 0;
    ...
    test('should handle network errors during booking', async ({ page }) => {
      ...
      const errorHandling = await page.locator('text=error, text=try again, text=connection').count() > 0;
  ...
  test.describe('Complete End-to-End Journey', () => {
    test('should complete full booking journey simulation', async ({ page }) => {
      ...
      await page.fill('input[placeholder*="City, hotel, landmark"]', 'Florence');
      ...
      if (await bookNowButtons.count() > 0) {
        ...
        const currentUrl = page.url();
        const hasBookingContent = await page.locator('form, .booking, [role="dialog"]').count() > 0;
        
        expect(currentUrl.includes('/booking') || currentUrl.includes('/payment') || hasBookingContent).toBe(true);
