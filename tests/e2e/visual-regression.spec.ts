...
test.describe('Visual Regression Testing - UI Consistency', () => {
  ...
  test.describe('Homepage Visual Consistency', () => {
    test('should maintain hero section visual consistency - Desktop', async ({ page }) => {
    ...
    test('should maintain hero section visual consistency - Mobile', async ({ page }) => {
    ...
    test('should maintain hero section visual consistency - Tablet', async ({ page }) => {
    ...
    test('should maintain booking widget visual consistency', async ({ page }) => {
      ...
      const bookingWidget = page.locator('.bg-white\\/95.backdrop-blur-lg.rounded-2xl');
      await expect(bookingWidget).toHaveScreenshot('booking-widget-desktop.png');
    });

    test('should maintain trust badges visual consistency', async ({ page }) => {
      await page.setViewportSize({ width: 1200, height: 800 });
      await page.goto('/');
      ...
      const trustBadges = page.locator('.flex.justify-center.items-center.gap-6.text-sm').first();
  ...
  test.describe('Search Results Visual Consistency', () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.waitForSelector('.space-y-6, [data-testid="search-results"]', { timeout: 15000 });
      await page.waitForLoadState('networkidle');
    ...
    test('should maintain search results layout - Desktop', async ({ page }) => {
      await page.setViewportSize({ width: 1200, height: 800 });
      
      // Screenshot search results header
      const resultsHeader = page.locator('.flex.flex-col.sm\\:flex-row.sm\\:items-center.sm\\:justify-between').first();
      await expect(resultsHeader).toHaveScreenshot('search-results-header-desktop.png');
      
      // Screenshot first hotel card
      const hotelCards = page.locator('.group.hover\\:shadow-2xl');
      if (await hotelCards.count() > 0) {
        await expect(hotelCards.first()).toHaveScreenshot('hotel-card-desktop.png');
      }
    });
    ...
    test('should maintain search results layout - Mobile', async ({ page }) => {
      ...
      const hotelCards = page.locator('.group.hover\\:shadow-2xl');
      if (await hotelCards.count() > 0) {
        await expect(hotelCards.first()).toHaveScreenshot('hotel-card-mobile.png');
      }
    });

    test('should maintain urgency indicators visual design', async ({ page }) => {
      await page.setViewportSize({ width: 1200, height: 800 });
      
      // Screenshot urgency indicators if present
      const urgencyIndicators = page.locator('[class*="animate-pulse"]:has-text("people viewing"), [class*="bg-orange-50"]');
      if (await urgencyIndicators.count() > 0) {
        await expect(urgencyIndicators.first()).toHaveScreenshot('urgency-indicator.png');
      }
    });
    ...
    test('should maintain deal badges visual design', async ({ page }) => {
      ...
      const dealBadges = page.locator('.bg-accent.text-white:has-text("OFF TODAY")');
      ...
      const featuredBanner = page.locator('[class*="bg-gradient-to-r"]:has-text("FEATURED DEAL")');
      if (await featuredBanner.count() > 0) {
        await expect(featuredBanner.first()).toHaveScreenshot('featured-deal-banner.png');
      }
    });

    test('should maintain Book Now button visual consistency', async ({ page }) => {
      await page.setViewportSize({ width: 1200, height: 800 });
      
      // Screenshot Book Now buttons
      const bookNowButtons = page.locator('button:has-text("Book Now")');
      if (await bookNowButtons.count() > 0) {
        await expect(bookNowButtons.first()).toHaveScreenshot('book-now-button-desktop.png');
      }
    });

    test('should maintain Book Now button mobile visual consistency', async ({ page }) => {
      await page.setViewportSize({ width: 375, height: 667 });
      
      const bookNowButtons = page.locator('button:has-text("Book Now")');
      if (await bookNowButtons.count() > 0) {
        await expect(bookNowButtons.first()).toHaveScreenshot('book-now-button-mobile.png');
      }
    });
  });
  ...
  test.describe('Testimonials Visual Consistency', () => {
    ...
    test('should maintain trust stats bar visual design', async ({ page }) => {
      ...
      const trustStatsBar = page.locator('.bg-white.rounded-2xl.shadow-lg.p-6').first();
      await expect(trustStatsBar).toHaveScreenshot('trust-stats-bar.png');
    });

    test('should maintain testimonials section header visual design', async ({ page }) => {
      await page.setViewportSize({ width: 1200, height: 800 });
      
      // Screenshot testimonials header section
      const headerSection = page.locator('.text-center.mb-12');
      if (await headerSection.count() > 0) {
        await expect(headerSection.first()).toHaveScreenshot('testimonials-header.png');
      }
    });

    test('should maintain desktop testimonials grid visual consistency', async ({ page }) => {
      await page.setViewportSize({ width: 1200, height: 800 });
      ...
      if (await desktopGrid.count() > 0) {
        await expect(desktopGrid).toHaveScreenshot('testimonials-desktop-grid.png');
      }
    });

    test('should maintain mobile testimonials carousel visual consistency', async ({ page }) => {
      await page.setViewportSize({ width: 375, height: 667 });
      ...
      const mobileCarousel = page.locator('.lg\\:hidden .bg-white.rounded-xl.shadow-lg.p-6');
      if (await mobileCarousel.count() > 0) {
        await expect(mobileCarousel).toHaveScreenshot('testimonials-mobile-carousel.png');
      }
      ...
      if (await navControls.count() > 0) {
        await expect(navControls).toHaveScreenshot('testimonials-mobile-nav.png');
      }
    });
  ...
  test.describe('UI Component Visual Consistency', () => {
    test('should maintain TrustBadge component visual design', async ({ page }) => {
      await page.goto('/');
      ...
      const trustBadges = page.locator('[class*="bg-primary-50"], [class*="bg-green-100"]');
      if (await trustBadges.count() > 0) {
        await expect(trustBadges.first()).toHaveScreenshot('trust-badge-component.png');
      }
    });

    test('should maintain UrgencyIndicator component visual design', async ({ page }) => {
      await page.goto('/');
      ...
      const urgencyIndicators = page.locator('[class*="bg-orange-50"], [class*="bg-green-50"], [class*="bg-red-50"]');
      if (await urgencyIndicators.count() > 0) {
        await expect(urgencyIndicators.first()).toHaveScreenshot('urgency-indicator-component.png');
      }
    });

    test('should maintain Button component variants visual consistency', async ({ page }) => {
      await page.goto('/');
      ...
      // Outline buttons
      const outlineButtons = page.locator('button:has-text("Browse Destinations")');
      if (await outlineButtons.count() > 0) {
        await expect(outlineButtons.first()).toHaveScreenshot('button-outline.png');
      }
    });

    test('should maintain Card component visual consistency', async ({ page }) => {
      await page.goto('/');
      await page.fill('input[placeholder*="City, hotel, landmark"]', 'Barcelona');
      await page.click('button:has-text("Search Hotels")');
      await page.waitForSelector('.space-y-6, [data-testid="search-results"]', { timeout: 15000 });
  ...
  test.describe('Cross-Browser Visual Consistency', () => {
    ...
    browsers.forEach(browserName => {
      test(`should maintain visual consistency in ${browserName}`, async ({ page }) => {
        ...
        const bookingWidget = page.locator('.bg-white\\/95.backdrop-blur-lg.rounded-2xl');
        await expect(bookingWidget).toHaveScreenshot(`booking-widget-${browserName}.png`);
      });
    });
  });

  test.describe('Loading State Visual Consistency', () => {
    test('should maintain search loading state visual design', async ({ page }) => {
      await page.goto('/');
      ...
      const loadingState = page.locator('text=Searching hotels..., .animate-pulse');
      if (await loadingState.count() > 0) {
        await expect(loadingState.first()).toHaveScreenshot('search-loading-state.png');
      }
  ...
  test.describe('Error State Visual Consistency', () => {
    test('should maintain error state visual design', async ({ page }) => {
      ...
      await page.fill('input[placeholder*="City, hotel, landmark"]', 'ErrorTest');
      await page.click('button:has-text("Search Hotels")');
