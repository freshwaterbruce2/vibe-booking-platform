...
test.describe('Search Results - Conversion-Focused Design', () => {
  ...
  test.describe('Search Results Layout and Structure', () => {
    test('should display search results with proper header', async ({ page }) => {
      ...
      // Verify sort options
      const sortOptions = ['relevance', 'price-low', 'price-high', 'rating', 'passion'];
      for (const option of sortOptions) {
        await expect(page.locator(`option[value="${option}"]`)).toBeVisible();
      }
    });

    test('should display hotel cards with conversion-focused layout', async ({ page }) => {
    ...
    test('should handle empty results gracefully', async ({ page }) => {
      ...
      await page.goto('/');
      await page.fill('input[placeholder*="City, hotel, landmark"]', 'NonExistentCity123456');
      await page.click('button:has-text("Search Hotels")');
  ...
  test.describe('Urgency Indicators and Social Proof', () => {
    ...
    test('should display passion match badges for high-scoring hotels', async ({ page }) => {
    ...
    test('should highlight featured deals properly', async ({ page }) => {
      ...
      if (await featuredBanners.count() > 0) {
        ...
        await expect(featuredBanners.first()).toContainText('Perfect Match for You');
      }
    });
  });
  ...
  test.describe('Book Now Buttons and CTAs', () => {
    test('should display prominent Book Now buttons', async ({ page }) => {
      ...
      await expect(bookNowButtons.first()).toHaveClass(/transform.*hover:scale-105/);
    });
    ...
    test('should show mobile-optimized CTAs', async ({ page }) => {
      ...
      const mobileBookButton = page.locator('button:has-text("Book Now")').first();
    ...
    test('should display View Details buttons', async ({ page }) => {
      ...
      if (await detailsButtons.count() > 0) {
        ...
        await expect(detailsButtons.first()).toHaveClass(/hover:text-primary-600/);
      }
    });
  });

  test.describe('Interactive Elements and Hover Effects', () => {
    test('should display action buttons on hover (desktop)', async ({ page, isMobile }) => {
      if (!isMobile) {
        const hotelCards = page.locator('.group.hover\\:shadow-2xl').first();
        await hotelCards.hover();
        
        // Check for favorite/heart button
        const heartButton = page.locator('button:has(svg)').filter({ has: page.locator('svg').filter({ hasText: 'Heart' }) });
        if (await heartButton.count() > 0) {
          await expect(heartButton.first()).toBeVisible();
        }
        
        // Check for share button
        const shareButton = page.locator('button:has(svg)').filter({ has: page.locator('svg').filter({ hasText: 'Share2' }) });
    ...
    test('should show virtual tour links when available', async ({ page }) => {
      ...
      const virtualTourButtons = page.locator('button:has-text("Virtual Tour")');
  ...
  test.describe('Mobile Responsiveness', () => {
    test('should adapt layout for mobile screens', async ({ page }) => {
      ...
      const hotelImages = page.locator('.w-full.lg\\:w-96.h-48.sm\\:h-56.lg\\:h-48');
    ...
    test('should stack elements properly on mobile', async ({ page }) => {
      ...
      const actionSections = page.locator('.flex.flex-col.sm\\:flex-row.sm\\:items-center');
  ...
  test.describe('Performance and Loading', () => {
    test('should load search results within performance budget', async ({ page }) => {
      ...
      await page.waitForSelector('.group.hover\\:shadow-2xl', { timeout: 10000 });
