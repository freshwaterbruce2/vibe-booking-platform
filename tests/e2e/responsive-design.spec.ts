...
test.describe('Responsive Design - Cross-Device Compatibility', () => {
    ...
    viewports.forEach(({ name, width, height, type }) => {
      test(`should display properly on ${name} (${width}x${height})`, async ({ page }) => {
        ...
        if (type === 'mobile') {
          ...
          const mobileDesc = page.locator('.md\\:hidden:has-text("Find perfect hotels with AI")');
        ...
        const destinationInput = page.locator('input[placeholder*="City, hotel, landmark"]');
  ...
  test.describe('Search Results Responsiveness', () => {
    ...
    viewports.forEach(({ name, width, height, type }) => {
      test(`should display search results properly on ${name} (${width}x${height})`, async ({ page }) => {
        await page.setViewportSize({ width, height });
        ...
        if (await hotelCards.count() > 0) {
          ...
          if (type === 'mobile') {
            ...
            const cardLayout = firstCard.locator('.flex.flex-col.lg\\:flex-row');
            ...
            const priceSection = firstCard.locator('.text-center.sm\\:text-right');
          ...
          const urgencyIndicators = firstCard.locator('[class*="text-orange-600"], [class*="text-green-600"], [class*="text-red-600"]');
          if (await urgencyIndicators.count() > 0) {
            await expect(urgencyIndicators.first()).toBeVisible();
          }

          // Test trust badges
          const trustBadges = firstCard.locator('[class*="bg-primary-50"], [class*="bg-green-100"]');
  ...
  test.describe('Testimonials Section Responsiveness', () => {
    ...
    viewports.forEach(({ name, width, height, type }) => {
      test(`should display testimonials properly on ${name} (${width}x${height})`, async ({ page }) => {
        await page.setViewportSize({ width, height });
        ...
        // Test main headline
        const headline = page.locator('h2:has-text("Real Stories from Real Travelers")');
        await expect(headline).toBeVisible();
        ...
        if (type === 'mobile') {
          ...
          const testimonialCards = desktopGrid.locator('.group .bg-white.rounded-2xl.shadow-lg');
  ...
  test.describe('Navigation Responsiveness', () => {
    viewports.forEach(({ name, width, height, type }) => {
      test(`should display navigation properly on ${name} (${width}x${height})`, async ({ page }) => {
        await page.setViewportSize({ width, height });
        await page.goto('/');
  ...
  test.describe('Content Readability and Typography', () => {
    viewports.forEach(({ name, width, height }) => {
      test(`should maintain readability on ${name} (${width}x${height})`, async ({ page }) => {
        await page.setViewportSize({ width, height });
        await page.goto('/');
        ...
        if (await textElements.count() > 0) {
          ...
          if (elementBox) {
            ...
            expect(elementBox.x + elementBox.width).toBeLessThanOrEqual(viewportWidth + 10); // 10px tolerance
          }
        }
      });
    });
  ...
  test.describe('Image and Media Responsiveness', () => {
    viewports.forEach(({ name, width, height, type }) => {
      test(`should display media properly on ${name} (${width}x${height})`, async ({ page }) => {
        await page.setViewportSize({ width, height });
        await page.goto('/');
        ...
        // Test hotel images
        const hotelImages = page.locator('img[src*="unsplash"], img[alt*="hotel"]');
  ...
  test.describe('Accessibility Across Devices', () => {
    viewports.forEach(({ name, width, height, type }) => {
      test(`should maintain accessibility on ${name} (${width}x${height})`, async ({ page }) => {
        await page.setViewportSize({ width, height });
        await page.goto('/');
        ...
        if (type !== 'mobile') { // Skip for mobile as touch is primary interaction
  ...
  test.describe('Layout Stability', () => {
    viewports.forEach(({ name, width, height }) => {
      test(`should maintain layout stability on ${name} (${width}x${height})`, async ({ page }) => {
        await page.setViewportSize({ width, height });
        await page.goto('/');
        ...
        if (initialHeadlinePos && finalHeadlinePos) {
          ...
          expect(Math.abs(initialHeadlinePos.y - finalHeadlinePos.y)).toBeLessThan(5);
        }

        if (initialWidgetPos && finalWidgetPos) {
          expect(Math.abs(initialWidgetPos.y - finalWidgetPos.y)).toBeLessThan(5);
        }

        if (initialButtonPos && finalButtonPos) {
          expect(Math.abs(initialButtonPos.y - finalButtonPos.y)).toBeLessThan(5);
        }
      });
    });
  });
