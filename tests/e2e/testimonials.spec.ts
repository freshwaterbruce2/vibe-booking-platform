...
test.describe('Testimonials Section - Social Proof and Trust Building', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      const testimonialsSection = document.querySelector('section:has(h2:contains("Real Stories"))') || 
  ...
  test.describe('Trust Stats Bar', () => {
    ...
    test('should display proper color coding for stats', async ({ page }) => {
      ...
      const primaryStat = page.locator('.text-primary').filter({ hasText: '4.9/5' });
      if (await primaryStat.count() > 0) {
        await expect(primaryStat).toBeVisible();
      }
      
      const accentStat = page.locator('.text-accent').filter({ hasText: '2M+' });
      if (await accentStat.count() > 0) {
        await expect(accentStat).toBeVisible();
      }
      
      const blueStat = page.locator('.text-blue-600').filter({ hasText: '99.9%' });
      if (await blueStat.count() > 0) {
        await expect(blueStat).toBeVisible();
      }
      
      const greenStat = page.locator('.text-green-600').filter({ hasText: '$15M+' });
  ...
  test.describe('Section Header and Branding', () => {
    test('should display featured badge and compelling headline', async ({ page }) => {
      // Check for featured badge
      const featuredBadge = page.locator('.bg-primary-50.text-primary-700:has-text("Featured Customer Stories")');
      await expect(featuredBadge).toBeVisible();
      
      // Check for checkmark icon in badge
      const checkIcon = page.locator('svg[fill="currentColor"]').filter({ has: page.locator('path[clip-rule="evenodd"]') });
      await expect(checkIcon.first()).toBeVisible();
      
      // Check main headline
      const mainHeadline = page.locator('h2:has-text("Real Stories from Real Travelers")');
      await expect(mainHeadline).toBeVisible();
      await expect(mainHeadline).toHaveClass(/text-4xl.*md:text-5xl.*font-bold/);
      
      // Check subheading
      const subheading = page.locator('text=See why millions choose Vibe Booking');
  ...
  test.describe('Desktop Testimonials Grid', () => {
    test('should display testimonials in grid layout on desktop', async ({ page }) => {
      await page.setViewportSize({ width: 1024, height: 768 });
      ...
      const testimonialCards = page.locator('.group .bg-white.rounded-2xl.shadow-lg');
      await expect(testimonialCards).toHaveCount(3);
    });

    test('should display featured story badge on first testimonial', async ({ page }) => {
      await page.setViewportSize({ width: 1024, height: 768 });
      
      // Check for featured story badge
      const featuredBadge = page.locator('.bg-primary.text-white:has-text("Featured Story")');
    ...
    test('should display complete testimonial information', async ({ page }) => {
      await page.setViewportSize({ width: 1024, height: 768 });
      
      const testimonialCards = page.locator('.group .bg-white.rounded-2xl.shadow-lg').first();
      ...
      // Check rating number
      const ratingNumber = testimonialCards.locator('.text-lg.font-bold.text-yellow-600');
      ...
      // Check user name
      const userName = testimonialCards.locator('.font-bold.text-gray-900.text-lg');
      ...
      const verifiedBadge = testimonialCards.locator('.bg-green-100.text-green-800:has-text("Verified Booking")');
      if (await verifiedBadge.count() > 0) {
        await expect(verifiedBadge).toBeVisible();
      }
      
      // Check hotel details
      const hotelDetails = testimonialCards.locator('.bg-gray-50.rounded-lg.p-3');
      await expect(hotelDetails).toBeVisible();
    });

    test('should handle hover effects on desktop', async ({ page, isMobile }) => {
      if (!isMobile) {
        await page.setViewportSize({ width: 1024, height: 768 });
        
        const testimonialCard = page.locator('.group .bg-white.rounded-2xl.shadow-lg').first();
        ...
        const hoverOverlay = testimonialCard.locator('.absolute.inset-0.bg-gradient-to-br');
    ...
    test('should handle avatar image loading errors', async ({ page }) => {
      ...
      const avatars = page.locator('img[alt*="Johnson"], img[alt*="Chen"], img[alt*="Rodriguez"]');
  ...
  test.describe('Mobile/Tablet Carousel', () => {
    ...
    test('should display current testimonial information in carousel', async ({ page }) => {
      await page.setViewportSize({ width: 375, height: 667 });
      ...
      // Check verified badge
      const verifiedBadge = carouselCard.locator('.bg-green-100.text-green-800:has-text("Verified")');
      if (await verifiedBadge.count() > 0) {
        await expect(verifiedBadge).toBeVisible();
      }
    ...
    test('should display navigation buttons', async ({ page }) => {
      ...
      const prevButton = page.locator('button').filter({ has: page.locator('svg path[d*="M15 19l-7-7 7-7"]') });
      await expect(prevButton).toBeVisible();
      await expect(prevButton).toHaveClass(/bg-white.*shadow-lg.*rounded-full/);
      
      // Check for next button
      const nextButton = page.locator('button').filter({ has: page.locator('svg path[d*="M9 5l7 7-7 7"]') });
    ...
    test('should handle carousel navigation', async ({ page }) => {
      ...
      const initialName = await page.locator('.font-semibold.text-gray-900').first().textContent();
      ...
      // Name should change
      const newName = await page.locator('.font-semibold.text-gray-900').first().textContent();
    ...
    test('should handle dot navigation', async ({ page }) => {
      ...
      const testimonialCard = page.locator('.bg-white.rounded-xl.shadow-lg.p-6');
  ...
  test.describe('Enhanced CTA Section', () => {
    test('should display compelling CTA with gradient background', async ({ page }) => {
      // Check for CTA section
      const ctaSection = page.locator('.bg-gradient-to-r.from-primary.to-primary-600.rounded-3xl');
      await expect(ctaSection).toBeVisible();
      
      // Check background pattern
      const backgroundPattern = ctaSection.locator('.absolute.inset-0.bg-white\\/5');
      await expect(backgroundPattern).toBeVisible();
      
      // Check main headline
      const headline = ctaSection.locator('h3:has-text("Ready to Find Your Perfect Hotel?")');
      await expect(headline).toBeVisible();
      await expect(headline).toHaveClass(/text-3xl.*md:text-4xl.*font-bold/);
      
      // Check value proposition
      const valueProp = ctaSection.locator('text=Join 2M+ travelers who saved money');
      await expect(valueProp).toBeVisible();
      
      // Check feature list
      const features = ctaSection.locator('text=• Free cancellation on most bookings • Best price guarantee • Instant confirmation');
    ...
    test('should display action buttons with proper styling', async ({ page }) => {
      ...
      // Primary CTA button
      const startBookingButton = ctaSection.locator('button:has-text("Start Booking Now")');
      await expect(startBookingButton).toBeVisible();
      await expect(startBookingButton).toHaveClass(/bg-white.*text-primary.*font-bold/);
      await expect(startBookingButton).toHaveClass(/hover:bg-gray-50.*transform.*hover:scale-105/);
      
      // Secondary CTA button
      const browseDealsButton = ctaSection.locator('button:has-text("Browse Deals")');
      await expect(browseDealsButton).toBeVisible();
      await expect(browseDealsButton).toHaveClass(/border-2.*border-white\/30.*text-white/);
    ...
    test('should display trust indicators in CTA', async ({ page }) => {
      ...
      const checkIcons = ctaSection.locator('svg[fill="currentColor"]').filter({ has: page.locator('path[clip-rule="evenodd"]') });
    ...
    test('should adapt CTA layout for mobile', async ({ page }) => {
      ...
      // Check button layout
      const buttonContainer = page.locator('.flex.flex-col.sm\\:flex-row.gap-4');
  ...
  test.describe('Performance and Accessibility', () => {
    test('should handle image loading errors gracefully', async ({ page }) => {
      ...
      const avatars = page.locator('img[alt*="Johnson"], img[alt*="Chen"], img[alt*="Rodriguez"], img[alt*="Kim"], img[alt*="Thompson"]');
    ...
    test('should meet accessibility standards', async ({ page }) => {
      ...
      const h2 = page.locator('h2:has-text("Real Stories from Real Travelers")');
      await expect(h2).toBeVisible();
      
      // Check for proper button labels
      const prevButton = page.locator('button[aria-label], button').filter({ has: page.locator('svg path[d*="M15 19l-7-7 7-7"]') });
      const nextButton = page.locator('button[aria-label], button').filter({ has: page.locator('svg path[d*="M9 5l7 7-7 7"]') });
    ...
    test('should load testimonials section within performance budget', async ({ page }) => {
      const startTime = Date.now();
      
      await page.goto('/');
      
      // Wait for testimonials section to be visible
      await page.waitForSelector('h2:has-text("Real Stories from Real Travelers")', { timeout: 5000 });
