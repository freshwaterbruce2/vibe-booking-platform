...
test.describe('Complete Booking Workflow', () => {
  ...
  test('should complete full booking workflow from search to confirmation', async ({ page }) => {
    ...
    await test.step('Select hotel from search results', async () => {
      // Click on hotel card
      await page.click('[data-testid="hotel-card"]:has-text("Grand Test Hotel")');

      // Should navigate to hotel details page
      await expect(page.url()).toContain('/hotels/hotel-123');
      await expect(page.locator('h1:has-text("Grand Test Hotel")')).toBeVisible();
    });
  ...
  test('should allow user to go back and modify booking details', async ({ page }) => {
  ...
  test('should maintain booking state across page refreshes', async ({ page }) => {
    ...
    await expect(page.locator(`[value="${mockBookingData.firstName}"]`)).toBeVisible();
    await expect(page.locator(`[value="${mockBookingData.email}"]`)).toBeVisible();
  });
  ...
  test('should show progress indicator throughout booking flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-step="room-selection"].active')).toBeVisible();
    ...
    await expect(page.locator('[data-step="room-selection"].completed')).toBeVisible();
    await expect(page.locator('[data-step="guest-details"].completed')).toBeVisible();
  });
...
test.describe('Search and Filter Workflow', () => {
  ...
  test('should sort hotels by different criteria', async ({ page }) => {
    ...
    const hotelNames = await page.locator('[data-testid="hotel-name"]').allTextContents();
    ...
    const hotelNamesAfterSort = await page.locator('[data-testid="hotel-name"]').allTextContents();
