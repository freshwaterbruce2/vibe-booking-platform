...
test.describe('Hero Section - Conversion-Focused Design', () => {
  ...
  test.describe('Video Background and Visual Elements', () => {
    test('should display video background with fallback', async ({ page }) => {
      ...
      const overlay = page.locator('.absolute.inset-0.bg-gradient-to-r.from-black\\/60');
  ...
  test.describe('Trust Badges and Social Proof', () => {
    test('should display trust badges in header', async ({ page }) => {
      ...
      await expect(page.locator('svg').filter({ hasText: 'Award' }).first()).toBeVisible();
      await expect(page.locator('svg').filter({ hasText: 'Clock' }).first()).toBeVisible();
    });

    test('should display trust statistics with proper formatting', async ({ page }) => {
      // Customer Rating
  ...
  test.describe('Mobile-First Booking Widget', () => {
    test('should display booking widget with proper layout', async ({ page }) => {
      // Check for booking widget container
      const widget = page.locator('.bg-white\\/95.backdrop-blur-lg.rounded-2xl');
    ...
    test('should show AI search section with proper styling', async ({ page }) => {
      // Check for AI badge
  ...
  test.describe('Conversion-Focused Headlines and CTAs', () => {
    test('should display compelling headlines with savings message', async ({ page }) => {
      // Main headline
    ...
    test('should display mobile-optimized descriptions', async ({ page, isMobile }) => {
      if (isMobile) {
        // Mobile description
        const mobileDesc = page.locator('text=Find perfect hotels with AI • Best prices • Instant booking');
        await expect(mobileDesc).toBeVisible();
      } else {
        // Desktop description
        const desktopDesc = page.locator('text=AI-powered hotel matching • Instant confirmation • Best prices guaranteed');
    ...
    test('should display action buttons with proper styling', async ({ page }) => {
      ...
      const browseButton = page.locator('button:has-text("Browse Destinations")');
  ...
  test.describe('Performance and Accessibility', () => {
    test('should load hero content within performance budgets', async ({ page }) => {
    ...
    test('should handle focus states properly', async ({ page }) => {
      ...
      const firstFocusable = page.locator('input[placeholder*="City, hotel, landmark"]');
  ...
  test.describe('Conversion Optimization Elements', () => {
    test('should highlight savings and value propositions', async ({ page }) => {
    ...
    test('should use urgency and scarcity elements appropriately', async ({ page }) => {
