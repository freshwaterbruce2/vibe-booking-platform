...
test.describe('Hotel Booking Performance Tests', () => {
  ...
  test.describe('Page Load Performance', () => {
    ...
    test('Search results should load efficiently', async ({ page }) => {
      ...
      // Perform search
      await page.fill('[placeholder="Where are you going?"]', 'Performance City');
  ...
  test.describe('Runtime Performance', () => {
    test('Search interaction should be responsive', async ({ page }) => {
      ...
      const destinationInput = page.locator('[placeholder="Where are you going?"]');
    ...
    test('Scrolling through search results should be smooth', async ({ page }) => {
      await page.goto('/');
      await page.fill('[placeholder="Where are you going?"]', 'Performance City');
      await page.click('button:has-text("Search Hotels")');
      await page.waitForSelector('text=Performance Test Hotel 1');
      ...
      const scrollMetrics = await page.evaluate(async () => {
        ...
        return new Promise((resolve) => {
          const measureFrame = () => {
            frameCount++;
            if (frameCount < 60) { // Measure for 60 frames (~1 second at 60fps)
    ...
    test('Hotel card hover interactions should be performant', async ({ page }) => {
      await page.goto('/');
      await page.fill('[placeholder="Where are you going?"]', 'Performance City');
      await page.click('button:has-text("Search Hotels")');
      await page.waitForSelector('text=Performance Test Hotel 1');
      ...
      const averageHoverTime = hoverTimes.reduce((a, b) => a + b, 0) / hoverTimes.length;
  ...
  test.describe('Memory Performance', () => {
    test('Search operations should not cause memory leaks', async ({ page }) => {
      await page.goto('/');
      ...
      for (let i = 0; i < 5; i++) {
        await page.fill('[placeholder="Where are you going?"]', `Test City ${i}`);
        await page.click('button:has-text("Search Hotels")');
        await page.waitForSelector('text=Performance Test Hotel 1');
    ...
    test('Long scrolling should not accumulate excessive DOM nodes', async ({ page }) => {
      await page.goto('/');
      await page.fill('[placeholder="Where are you going?"]', 'Performance City');
      await page.click('button:has-text("Search Hotels")');
      await page.waitForSelector('text=Performance Test Hotel 1');
      ...
      console.log(`DOM node increase: ${nodeIncrease} nodes (${nodeIncreasePercent.toFixed(2)}%)`);
  ...
  test.describe('Network Performance', () => {
    ...
    test('Should handle slow network gracefully', async ({ page }) => {
      ...
      // Wait for results
      await page.waitForSelector('text=Performance Test Hotel 1', { timeout: 10000 });
  ...
  test.describe('Bundle Size and Asset Performance', () => {
    test('Should load critical resources efficiently', async ({ page }) => {
      ...
      const largeResources = performanceData.filter(resource => resource.size > 500000); // >500KB
    ...
    test('Should optimize image loading', async ({ page }) => {
      ...
      const loadedPercentage = (loadedImages.length / imageMetrics.length) * 100;
  ...
  test.describe('User Experience Performance', () => {
    test('Should provide fast feedback for user interactions', async ({ page }) => {
      await page.goto('/');
    ...
    test('Should maintain 60fps during animations', async ({ page }) => {
      ...
      const animationMetrics = await page.evaluate(async () => {
        return new Promise((resolve) => {
          ...
          // Trigger animation
          const hotelCard = document.querySelector('[data-testid="hotel-card"]') as HTMLElement;
          if (hotelCard) {
  ...
  test.describe('Error Handling Performance', () => {
    test('Should handle API errors without blocking UI', async ({ page }) => {
      ...
      const input = await page.inputValue('[placeholder="Where are you going?"]');
