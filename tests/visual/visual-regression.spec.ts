          ...
          hotels: [
            {
              ...
              description: 'Luxury hotel in downtown with stunning city views and world-class amenities.',
              location: {
                city: 'Visual City',
            ...
            },
            {
              ...
              description: 'Clean, comfortable accommodation with essential amenities for budget-conscious travelers.',
              location: {
                city: 'Visual City',
                country: 'Test Country',
...
test.describe('Hotel Booking Visual Regression Tests', () => {
  ...
  test.describe('Search Functionality Visual Tests', () => {
    test('Search form initial state', async ({ page }) => {
      ...
      const searchSection = page.locator('section:has(input[placeholder*="Where are you going"])');
    ...
    test('Individual hotel card design', async ({ page }) => {
      ...
      const luxuryHotelCard = page.locator('[data-testid="hotel-card"]').first();
    ...
    test('Empty search results', async ({ page }) => {
      ...
      await page.goto('/');
      await page.fill('[placeholder="Where are you going?"]', 'Nonexistent City');
      await page.click('button:has-text("Search Hotels")');
      
      await page.waitForSelector('text=No hotels found');
      
      const emptyState = page.locator('div:has-text("No hotels found")').first();
  ...
  test.describe('Responsive Design Visual Tests', () => {
    VIEWPORTS.forEach(({ name: viewportName, width, height }) => {
      test(`Search results responsive - ${viewportName}`, async ({ page }) => {
        ...
        await expect(page).toHaveScreenshot(`search-results-${viewportName}.png`, {
          fullPage: true,
          animations: 'disabled',
        });
      });
  ...
  test.describe('Cross-browser Compatibility', () => {
    ...
    browsers.forEach((browserName) => {
      test(`Homepage consistency - ${browserName}`, async ({ page, browserName: currentBrowser }) => {
      ...
      test(`Search results consistency - ${browserName}`, async ({ page, browserName: currentBrowser }) => {
        test.skip(currentBrowser !== browserName);
        ...
        await expect(page).toHaveScreenshot(`search-results-${browserName}.png`, {
          fullPage: true,
          animations: 'disabled',
          threshold: 0.3,
        });
