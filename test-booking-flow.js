...
async function testBookingFlow() {
  ...
  try {
    ...
    const searchSection = await page
      .locator('[data-testid="search-section"], .search-section, form')
      .first();
    if (await searchSection.isVisible()) {
      ...
      const cityInput = page
        .locator(
          'input[placeholder*="city"], input[placeholder*="destination"], input[name="destination"]',
        )
        .first();
      ...
      const checkinInput = page
        .locator('input[type="date"], input[placeholder*="check"], input[name*="checkin"]')
        .first();
      ...
      const checkoutInput = page
        .locator('input[type="date"], input[placeholder*="check"], input[name*="checkout"]')
        .nth(1);
      ...
      const searchButton = page
        .locator('button[type="submit"], button:has-text("Search"), .search-button')
        .first();
      if (await searchButton.isVisible()) {
        ...
        const hotelCards = page.locator(
          '.hotel-card, [data-testid="hotel-card"], .hotel-item, .result-item',
        );
        ...
        if (cardCount > 0) {
          ...
          const bookButton = page
            .locator(
              'button:has-text("Book"), button:has-text("Reserve"), button:has-text("Select"), .book-button, .reserve-button',
            )
            .first();
          if (await bookButton.isVisible()) {
            ...
            const proceedButton = page
              .locator(
                'button:has-text("Payment"), button:has-text("Proceed"), button:has-text("Continue"), button:has-text("Next")',
              )
              .first();
              ...
              if (squareFound) {
                ...
                try {
                  ...
                  const cardInput = page
                    .locator(
                      'input[placeholder*="Card"], input[data-field="cardNumber"], #card-number',
                    )
                    .first();
                  if (await cardInput.isVisible()) {
                    ...
                    const expiryInput = page
                      .locator(
                        'input[placeholder*="MM/YY"], input[data-field="expirationDate"], #card-expiry',
                      )
                      .first();
                    ...
                    const cvvInput = page
                      .locator('input[placeholder*="CVV"], input[data-field="cvv"], #card-cvv')
                      .first();
                    ...
                    await page.screenshot({
                      path: 'screenshots/08-payment-form-filled.png',
                      fullPage: true,
                    });
                    ...
                    const payButton = page
                      .locator(
                        'button:has-text("Pay"), button:has-text("Complete"), button:has-text("Submit"), .pay-button',
                      )
                      .first();
                    if (await payButton.isVisible()) {
                      console.log(
                        '📍 Step 11: Payment button found - ready for payment submission',
                      );
                      await page.screenshot({
                        path: 'screenshots/09-ready-for-payment.png',
                        fullPage: true,
                      });
                  ...
                  await page.screenshot({
                    path: 'screenshots/08-payment-error.png',
                    fullPage: true,
                  });
                }
              } else {
                ...
                const genericPayment = page
                  .locator(
                    'input[type="text"], input[placeholder*="card"], .payment-form, .card-form',
                  )
                  .first();
