...
async function testPaymentStatus() {
  ...
  try {
    ...
    const isDemoMode =
      pageText.includes('Demo Payment Mode') ||
      pageText.includes('Test Card Numbers') ||
      pageText.includes('Demo mode');

    console.log(
      `💳 Current Status: ${isDemoMode ? '❌ Demo Mode Active' : '✅ Live Payments Active'}`,
    );
...
async function finalPaymentTest() {
  ...
  try {
    ...
    console.log(
      `🎯 FINAL RESULT: ${isLive ? '✅ LIVE PAYMENTS ACTIVE!' : '⏳ Waiting for activation'}`,
    );
