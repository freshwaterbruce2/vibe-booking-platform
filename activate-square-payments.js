...
async function activateSquarePayments() {
  ...
  try {
    ...
    const triggerFiles = ['restart-server.txt', 'package.json', '.ionos-deploy'];
    ...
    try {
      ...
      await page.click(
        'button:has-text("Book"), button:has-text("Select"), .hotel-card:first-child button',
      );
      ...
      if (await guestForm.isVisible()) {
        ...
        const paymentBtn = page.locator(
          'button:has-text("Payment"), button:has-text("Continue"), button:has-text("Next")',
        );
    ...
    const isDemoMode =
      demoModeText.includes('Demo Payment Mode') ||
    ...
    if (isDemoMode) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const stillDemo =
          currentText.includes('Demo Payment Mode') || currentText.includes('Demo mode');
    ...
    const finalDemo =
      finalText.includes('Demo Payment Mode') || finalText.includes('Test Card Numbers');
