import { a as R, u as B, j as e, c as S, I as F, B as k, l as j } from './index-BipxYy_H.js';
import { r as b, u as M } from './vendor-VgjaNQN2.js';
import {
  h as I,
  w as H,
  S as T,
...
import { O as C } from './OptimizedImage-aUUc6jy1.js';
class Y {
  baseURL = 'https://api.vibehotels.com';
  async processNaturalLanguage(h) {
    try {
      return (
        await R.post(`${this.baseURL}/ai/process-query`, {
          query: h,
      ...
      throw (
        console.error('Error processing natural language query:', o),
  ...
  async getRecommendations(h) {
    try {
      return (await R.post(`${this.baseURL}/ai/recommendations`, h)).data;
    } catch (o) {
      throw (
        console.error('Error getting AI recommendations:', o),
  ...
  async analyzeSentiment(h) {
    try {
      return (await R.post(`${this.baseURL}/ai/sentiment`, { text: h })).data;
    } catch (o) {
      throw (
        console.error('Error analyzing sentiment:', o),
...
const K = new Y();
function V({ className: i, placeholder: h = 'Describe your perfect trip...', size: o = 'lg' }) {
  const [t, d] = b.useState(!1),
    [l, x] = b.useState(!1),
    {
      naturalLanguageQuery: u,
      setNaturalLanguageQuery: a,
      setAiProcessedQuery: p,
      setDateRange: n,
      setGuestCount: s,
      setFilters: m,
    } = B(),
    g = async () => {
      if (u.trim()) {
        d(!0);
        try {
          const c = await K.processNaturalLanguage(u);
          (p(c),
            c.extractedDetails.dates &&
              n(c.extractedDetails.dates.checkIn, c.extractedDetails.dates.checkOut),
            c.extractedDetails.guests &&
              s(
                c.extractedDetails.guests.adults,
                c.extractedDetails.guests.children,
                c.extractedDetails.guests.rooms,
              ),
            c.extractedDetails.budget &&
              m({ priceRange: [c.extractedDetails.budget.min, c.extractedDetails.budget.max] }));
        } catch (c) {
          console.error('Error processing natural language query:', c);
        } finally {
          d(!1);
        }
      }
    ...
    f = () => {
      ...
      ((r.continuous = !1),
        ...
        (r.onresult = (P) => {
          const { transcript: v } = P.results[0][0];
          (a(v), x(!1));
        }),
        (r.onerror = () => {
          x(!1);
    ...
    w = { sm: 'h-10 text-sm', md: 'h-12 text-base', lg: 'h-14 text-lg' };
  return e.jsxs('div', {
    className: S('relative w-full max-w-2xl', i),
    children: [
      e.jsxs('div', {
        className: 'relative',
        children: [
          e.jsx('div', {
            ...
            children: e.jsx(I, {
              className: S(
                'transition-colors',
                o === 'sm' ? 'h-4 w-4' : o === 'md' ? 'h-5 w-5' : 'h-6 w-6',
          ...
          e.jsxs('div', {
            ...
            children: [
              e.jsx(k, {
                ...
                onClick: f,
                disabled: l || t,
                className: S('h-8 w-8', l && 'text-red-500 animate-pulse'),
...
function Q() {
    ...
    a = async (p) => {
      ...
      try {
        ...
        if ('https://api.vibehotels.com'.includes('localhost'))
          throw (
            j.info('No backend deployed - using mock data for instant results', {
        ...
        const g = await R.post(
          ...
          w = (g.data.data || g.data).hotels || g.data.hotels || [];
        if (g.data.success && w && w.length > 0) {
          const c = w.map((r, P) => ({
            id: r.id || String(P + 1),
            name: r.name || 'Unnamed Hotel',
            description: r.description || 'A comfortable place to stay',
            location: {
              address: r.address || '123 Main St',
              city: r.city || n,
            ...
            priceRange: {
              min: r.pricePerNight || 100,
              max: (r.pricePerNight || 100) * 1.5,
              currency: r.currency || 'USD',
              avgNightly: r.pricePerNight || 100,
            },
            images: r.images
              ? r.images.map((v, y) => ({
                  id: `img-${y}`,
                  url: v,
                  alt: `${r.name} - Image ${y + 1}`,
                  category: y === 0 ? 'exterior' : 'room',
                  isPrimary: y === 0,
                }))
              : [
                  {
                    ...
                    alt: r.name || 'Hotel Image',
                    category: 'exterior',
                    isPrimary: !0,
                  },
                ],
            amenities: (r.amenities || ['WiFi', 'Parking']).map((v, y) => ({
              id: `amenity-${y}`,
              name: v,
              category: 'general',
              icon: void 0,
            ...
            passionScore: r.passions
              ? r.passions.reduce((v, y) => ((v[y] = Math.random() * 100), v), {})
              : {},
            sustainabilityScore: r.sustainabilityCertified ? 85 : void 0,
          }));
          l(c);
        } else {
          j.info('API returned no hotels, using comprehensive fallback data');
          const c = [
            {
              id: '1',
              name: `Grand Hotel ${n}`,
        ...
        j.warn('Hotel search failed, providing fallback data for seamless UX', {
          component: 'useHotelSearch',
          destination: n.trim(),
  ...
  return (
    ...
    { searchHotels: a, isSearching: i }
  );
}
function J() {
  const i = M(),
    { setQuery: h, setDateRange: o } = B(),
    { searchHotels: t } = Q(),
    [d, l] = b.useState(''),
    [x, u] = b.useState(''),
    [a, p] = b.useState(''),
    n = () => {
      (j.info('Search initiated from hero section', {
        component: 'Hero',
        destination: d.trim(),
        hasCheckIn: !!x,
        hasCheckOut: !!a,
        ...
        d.trim()
          ? (h(d.trim()),
            ...
            i('/search'),
            t(d.trim()).catch((s) => {
              j.error('Hero search failed, user will see fallback results', {
                component: 'Hero',
                destination: d.trim(),
                error: s instanceof Error ? s.message : 'Unknown error',
              });
            }))
          : (j.warn('Search attempted without destination', { component: 'Hero' }),
            ...
            e.jsxs('div', {
              ...
              children: [
                e.jsxs(k, {
                  ...
                  children: [e.jsx(q, { className: 'mr-2 h-5 w-5' }), 'Watch Demo'],
                }),
                e.jsx(k, {
                  size: 'lg',
                  variant: 'outline',
                ...
                e.jsxs('div', {
                  ...
                  children: [
                    e.jsxs('div', {
                      className: 'flex items-center justify-center mb-2',
                      children: [
                        e.jsx(_, { className: 'h-6 w-6 text-accent mr-2' }),
...
const X = ({
  ...
  Z = ({
    ...
    const [l, x] = b.useState(i),
      ...
      a = (s) => {
        const m = l.includes(s) ? l.filter((g) => g !== s) : [...l, s];
        (x(m), h && h(s));
      },
      p = () => {
        o && o(l);
    ...
    return t
      ? e.jsxs('section', {
          className: `relative bg-gradient-to-br from-slate-50 via-white to-amber-50/30 py-20 overflow-hidden ${d}`,
          children: [
            e.jsxs('div', {
                      ...
                      e.jsxs('div', {
                        ...
                        children: [
                          e.jsxs('div', {
                            ...
                            children: [
                              e.jsx('div', {
                                className: `w-3 h-3 rounded-full transition-colors duration-300 ${l.length > 0 ? 'bg-slate-600' : 'bg-slate-300'}`,
                              }),
                              e.jsxs('p', {
  ...
  ee = ({
    ...
    const [t, d] = b.useState(0),
      l = () => {
        d((a) => (a === i.length - 1 ? 0 : a + 1));
      },
      x = () => {
        d((a) => (a === 0 ? i.length - 1 : a - 1));
      },
      u = (a) =>
        [...Array(5)].map((p, n) =>
          e.jsx(
    ...
    return e.jsx('section', {
      className: `py-20 bg-gradient-to-b from-white to-gray-50 ${o}`,
      children: e.jsxs('div', {
        className: 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8',
        children: [
          e.jsx('div', {
                            ...
                            e.jsxs('div', {
                              ...
                              children: [
                                e.jsx('p', {
                                  ...
                                  children: i[t].hotelStayed,
                                }),
                                e.jsx('p', {
                                  className: 'text-xs opacity-90',
                                  children: i[t].dateStayed,
                                }),
                              ],
                            }),
                          ],
              ...
              e.jsx('div', {
                ...
                children: i.map((a, p) =>
                  e.jsx(
                    ...
                    {
                      onClick: () => d(p),
                      className: `w-2 h-2 rounded-full transition-colors ${p === t ? 'bg-blue-600' : 'bg-gray-300'}`,
                    },
                    p,
                  ),
                ),
...
function ie() {
  return e.jsxs('div', {
    className: 'space-y-0',
    children: [e.jsx(J, {}), e.jsx(Z, {}), e.jsx(X, {}), e.jsx(ee, {})],
  });
}
export { ie as HomePage };
//# sourceMappingURL=HomePage-uismDQNG.js.map
