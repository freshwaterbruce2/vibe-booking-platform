import { initializeSqliteDatabase, getSqliteConnection, closeSqliteDatabase } from '../sqlite';
  ...
  try {
    ...
    for (const user of users) {
      insertUser.run(
        ...
        user.role, user.isActive, user.emailVerified, user.phone,
    ...
    const hotels = [
      {
        ...
        priceMax: 899.00,
        currency: 'USD',
        amenities: JSON.stringify(['wifi', 'parking', 'pool', 'spa', 'restaurant', 'bar', 'gym', 'concierge']),
        images: JSON.stringify(['/images/hotel1-1.jpg', '/images/hotel1-2.jpg', '/images/hotel1-3.jpg']),
        policies: JSON.stringify({ checkIn: '15:00', checkOut: '11:00', cancellation: '24h' }),
        phone: '+1-555-0200',
      ...
      },
      {
        ...
        description: 'Beautiful beachfront resort perfect for relaxation and adventure, featuring ocean views and world-class spa services.',
        ...
        priceMax: 599.00,
        currency: 'USD',
        amenities: JSON.stringify(['wifi', 'beach', 'pool', 'spa', 'restaurant', 'bar', 'water-sports', 'kids-club']),
        images: JSON.stringify(['/images/hotel2-1.jpg', '/images/hotel2-2.jpg', '/images/hotel2-3.jpg']),
        policies: JSON.stringify({ checkIn: '16:00', checkOut: '11:00', cancellation: '48h' }),
        phone: '+1-555-0201',
      ...
      },
      {
        ...
        priceMax: 399.00,
        currency: 'USD',
        amenities: JSON.stringify(['wifi', 'business-center', 'meeting-rooms', 'gym', 'restaurant', 'parking']),
        images: JSON.stringify(['/images/hotel3-1.jpg', '/images/hotel3-2.jpg', '/images/hotel3-3.jpg']),
        policies: JSON.stringify({ checkIn: '15:00', checkOut: '12:00', cancellation: '24h' }),
        phone: '+1-555-0202',
    ...
    for (const hotel of hotels) {
      insertHotel.run(
        hotel.id, hotel.externalId, hotel.name, hotel.slug, hotel.description, hotel.shortDescription,
        hotel.address, hotel.city, hotel.state, hotel.country, hotel.postalCode, 
        hotel.latitude, hotel.longitude, hotel.neighborhood,
        hotel.rating, hotel.reviewCount, hotel.starRating, hotel.priceMin, hotel.priceMax, hotel.currency,
        hotel.amenities, hotel.images, hotel.policies, hotel.phone, hotel.email, hotel.website, hotel.totalRooms,
        hotel.isActive, hotel.isFeatured, hotel.passionScores,
        Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000)
      );
    }
    ...
    const rooms = [
      // Grand Palace Hotel rooms
      {
        ...
        type: 'suite',
        description: 'Spacious executive suite with city view and premium amenities',
        maxOccupancy: 4,
        adults: 2,
        children: 2,
        basePrice: 599.00,
        size: 750,
        bedConfiguration: JSON.stringify([{ type: 'king', count: 1 }, { type: 'sofa_bed', count: 1 }]),
        amenities: JSON.stringify(['wifi', 'minibar', 'safe', 'city-view', 'work-desk', 'premium-bedding'])
      },
      {
        id: crypto.randomUUID(),
        hotelId: hotels[0].id,
      ...
      // Seaside Resort rooms
      {
        ...
        size: 650,
        bedConfiguration: JSON.stringify([{ type: 'king', count: 1 }, { type: 'sofa_bed', count: 1 }]),
        amenities: JSON.stringify(['wifi', 'ocean-view', 'balcony', 'minibar', 'safe', 'premium-bedding'])
      },
      {
        id: crypto.randomUUID(),
        hotelId: hotels[1].id,
    ...
    for (const room of rooms) {
      insertRoom.run(
        room.id, room.hotelId, room.externalId, room.name, room.type, room.description,
        room.maxOccupancy, room.adults, room.children, room.basePrice, room.size,
        room.bedConfiguration, room.amenities,
        Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000)
      );
    }
    ...
    for (const booking of bookings) {
      insertBooking.run(
        booking.id, booking.confirmationNumber, booking.userId,
        booking.guestEmail, booking.guestFirstName, booking.guestLastName, booking.guestPhone,
        booking.hotelId, booking.roomId, booking.checkIn, booking.checkOut, booking.nights,
        booking.adults, booking.children, booking.roomRate, booking.taxes, booking.fees,
        booking.totalAmount, booking.status, booking.paymentStatus, booking.specialRequests,
        Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000)
      );
    }
    ...
    for (const payment of payments) {
      insertPayment.run(
        ...
        payment.status, payment.method, payment.provider, payment.transactionId,
        payment.cardLast4, payment.cardBrand, payment.cardExpMonth, payment.cardExpYear,
        payment.processedAt, Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000)
      );
    }
    ...
    const reviews = [
      {
        ...
        comment: 'The Grand Palace Hotel exceeded all expectations. The staff was incredibly friendly and professional, the room was spotless and luxurious, and the location was perfect for exploring the city. I would definitely stay here again!',
    ...
    for (const review of reviews) {
      insertReview.run(
        review.id, review.hotelId, review.userId, review.bookingId,
        review.reviewerName, review.reviewerEmail, review.isVerifiedBooking,
        review.overallRating, review.cleanliness, review.location, review.service,
        review.value, review.comfort, review.facilities, review.title, review.comment,
        review.pros, review.cons, review.tripType, review.stayDate, review.roomType,
        review.status, review.sentiment, review.sentimentScore,
        Math.floor(Date.now() / 1000), Math.floor(Date.now() / 1000)
      );
    }

    logger.info('Database seeding completed successfully!');
    logger.info(`Seeded ${users.length} users, ${hotels.length} hotels, ${rooms.length} rooms, ${bookings.length} bookings, ${payments.length} payments, and ${reviews.length} reviews`);
