import { pgTable, text, timestamp, varchar, numeric, jsonb, uuid, index, integer, boolean, unique, check } from 'drizzle-orm/pg-core';
...
export const bookings = pgTable('bookings', {
  ...
  return {
    ...
    statusesIdx: index('bookings_statuses_idx').on(table.status, table.paymentStatus),
    ...
    guestNameIdx: index('bookings_guest_name_idx').on(table.guestFirstName, table.guestLastName),
    ...
    hotelRevenueIdx: index('bookings_hotel_revenue_idx').on(table.hotelId, table.totalAmount, table.createdAt),
    
    // Unique constraints
    confirmationUnique: unique('bookings_confirmation_unique').on(table.confirmationNumber),
    
    // Data validation constraints
    dateValidation: check('bookings_date_validation', sql`${table.checkOut} > ${table.checkIn}`),
    nightsValidation: check('bookings_nights_validation', sql`${table.nights} > 0`),
    guestsValidation: check('bookings_guests_validation', sql`${table.adults} > 0 AND ${table.adults} + ${table.children} <= 10`),
    amountValidation: check('bookings_amount_validation', sql`${table.totalAmount} > 0`),
  };
});
...
export type BookingStatusHistory = z.infer<typeof selectBookingStatusHistorySchema>;
export type NewBookingStatusHistory = z.infer<typeof insertBookingStatusHistorySchema>;
