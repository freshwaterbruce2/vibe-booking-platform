 ...
 */
export const hotelSearch = pgTable('hotel_search', {
  id: uuid('id').defaultRandom().primaryKey(),
  hotelId: uuid('hotel_id').notNull().references(() => hotels.id, { onDelete: 'cascade' }).unique(),
  ...
  return {
    ...
    nameVectorIdx: index('hotel_search_name_vector_idx').using('gin', sql`to_tsvector('english', ${table.nameVector})`),
    descriptionVectorIdx: index('hotel_search_description_vector_idx').using('gin', sql`to_tsvector('english', ${table.descriptionVector})`),
    locationVectorIdx: index('hotel_search_location_vector_idx').using('gin', sql`to_tsvector('english', ${table.locationVector})`),
    amenitiesVectorIdx: index('hotel_search_amenities_vector_idx').using('gin', sql`to_tsvector('english', ${table.amenitiesVector})`),
    combinedVectorIdx: index('hotel_search_combined_vector_idx').using('gin', sql`to_tsvector('english', ${table.combinedVector})`),
    
    // Relevance scoring indexes
    popularityIdx: index('hotel_search_popularity_idx').on(table.popularityScore),
    qualityIdx: index('hotel_search_quality_idx').on(table.qualityScore),
    locationIdx: index('hotel_search_location_score_idx').on(table.locationScore),
    
    // Combined scoring for ranking
    scoringIdx: index('hotel_search_scoring_idx').on(table.popularityScore, table.qualityScore, table.locationScore),
    
    // Keyword tags (GIN for JSONB operations)
    keywordTagsIdx: index('hotel_search_keyword_tags_idx').using('gin', table.keywordTags),
  };
});
...
export const searchAnalytics = pgTable('search_analytics', {
  ...
  return {
    ...
    queryTypeIdx: index('search_analytics_query_type_idx').on(table.queryType),
    normalizedQueryIdx: index('search_analytics_normalized_query_idx').on(table.normalizedQuery),
    
    // Performance tracking
    executionTimeIdx: index('search_analytics_execution_time_idx').on(table.executionTimeMs),
    ...
    dailyIdx: index('search_analytics_daily_idx').on(sql`date_trunc('day', ${table.createdAt})`),
  };
});
