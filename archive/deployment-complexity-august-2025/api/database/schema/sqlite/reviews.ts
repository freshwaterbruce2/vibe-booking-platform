...
export const reviews = sqliteTable('reviews', {
  ...
  isVerifiedBooking: integer('is_verified_booking', { mode: 'boolean' }).notNull().default(false),
...
export const reviewVotes = sqliteTable('review_votes', {
  ...
  ipAddress: text('ip_address', { length: 45 }),
  voteType: text('vote_type', { length: 10 }).notNull(), // helpful, not_helpful
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull().$defaultFn(() => new Date()),
}, (table) => {
  return {
    reviewUserIdx: index('review_votes_review_user_idx').on(table.reviewId, table.userId),
    reviewIpIdx: index('review_votes_review_ip_idx').on(table.reviewId, table.ipAddress),
  };
});
...
export type ReviewVote = z.infer<typeof selectReviewVoteSchema>;
export type NewReviewVote = z.infer<typeof insertReviewVoteSchema>;
export type ReviewReport = z.infer<typeof selectReviewReportSchema>;
export type NewReviewReport = z.infer<typeof insertReviewReportSchema>;