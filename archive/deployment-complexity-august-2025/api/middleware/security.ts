...
export const securityMiddleware = [
  ...
  (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.path.includes('/payment') || req.path.includes('/admin') || 
        req.path.includes('/auth') || req.path.includes('/user')) {
...
export const auditLogger = (req: Request, res: Response, next: NextFunction) => {
  ...
  const isSensitiveRoute = req.path.includes('/payment') || req.path.includes('/admin');
 ...
 */
export const paymentCors = (req: Request, res: Response, next: NextFunction) => {
  ...
  const checkForCardData = (obj: any, path = ''): string | null => {
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === 'string') {
        ...
        // CVV pattern
        if (key.toLowerCase().includes('cvv') || key.toLowerCase().includes('cvc')) {
          return `CVV/CVC data detected at ${currentPath}`;
        }
        
        // Expiry pattern
        if (/^\d{2}\/\d{2}$/.test(value) && (key.toLowerCase().includes('exp') || key.toLowerCase().includes('date'))) {
  ...
  if (req.body) {
    ...
    if (cardDataFound) {
      ...
      return res.status(400).json({
        ...
        message: 'Sensitive payment information should not be sent directly to this endpoint',
      });
    }
  }
