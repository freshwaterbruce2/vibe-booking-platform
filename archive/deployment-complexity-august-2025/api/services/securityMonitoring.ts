...
import { securityIncidents, auditLog, userActivityLog } from '../database/schema';
...
export class SecurityMonitoringService extends EventEmitter {
  ...
  async reportSecurityEvent(event: {
    ...
    try {
      ...
      const [securityEvent] = await db.insert(securityIncidents).values({
  ...
  private async checkUnusualUserActivity(since: Date): Promise<void> {
    ...
    for (const userPattern of activeUsers) {
      const severity = userPattern.distinctIPs && userPattern.distinctIPs > 5 ? 'HIGH' : 'MEDIUM';
  ...
  private async checkPaymentAnomalies(since: Date): Promise<void> {
    ...
    if (failures && failures.failure_count >= this.alertThresholds.payment_failures) {
      await this.reportSecurityEvent({
   ...
   */
  private async handleHighSeverityIncident(incidentId: string, event: any): Promise<void> {
    try {
      ...
      logger.error('Failed to handle high-severity incident', { error, incidentId });
    }
  }
  ...
  private async generateDailySecurityReport(): Promise<void> {
    try {
      ...
      const incidentSummary = await db
        ...
        .where(and(
          gte(securityIncidents.createdAt, yesterday),
          lte(securityIncidents.createdAt, today)
        ))
      ...
      const report = {
        ...
        totalIncidents: incidentSummary.reduce((sum, item) => sum + item.count, 0),
  ...
  async getSecurityMetrics(timeframe: 'hour' | 'day' | 'week' = 'day'): Promise<any> {
    try {
      ...
      const incidents = await db
        ...
        .from(securityIncidents)
        .where(gte(securityIncidents.createdAt, since))
        .groupBy(securityIncidents.severity, securityIncidents.incidentType);
