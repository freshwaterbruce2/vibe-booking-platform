...
import { users, auditLog, userActivityLog, dataAccessLogs } from '../database/schema';
...
export class GDPRCompliance {
  ...
  static async exportUserData(userId: string): Promise<{
    ...
    try {
      ...
      await this.logDataAccess(userId, 'EXPORT', 'COMPLETE_PROFILE', 'Data export requested by user');
      ...
      const exportData = {
        ...
        metadata: {
          ...
          contact: 'For questions about this export, contact privacy@hotelbooking.com',
        },
      };
  ...
  static async deleteUserData(userId: string, reason: string): Promise<{
    ...
    try {
      ...
      await this.logDataAccess(userId, 'DELETE', 'COMPLETE_PROFILE', `Data deletion requested: ${reason}`);

      // Start transaction for atomic deletion
      const deletionSummary: Record<string, number> = {};

      // Delete user activity logs
      const activityResult = await db.delete(userActivityLog).where(eq(userActivityLog.userId, userId));
  ...
  static async rectifyUserData(
    ...
    try {
      ...
      await this.logDataAccess(userId, 'UPDATE', 'PROFILE', 'Data rectification requested by user');
   ...
   */
  static async restrictProcessing(userId: string, reason: string): Promise<boolean> {
    try {
      const db = getDb();
      ...
      await this.logDataAccess(userId, 'RESTRICT', 'PROFILE', `Processing restricted: ${reason}`);
   ...
   */
  static async validateConsent(userId: string, processingPurpose: string): Promise<boolean> {
    try {
      const db = getDb();
      ...
      logger.error('Failed to validate consent', { error, userId, processingPurpose });
      return false;
    }
  }
  ...
  static async updateConsent(
    ...
    try {
      ...
      await this.logDataAccess(userId, 'UPDATE', 'CONSENT', 'User consent preferences updated');
