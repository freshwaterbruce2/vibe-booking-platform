...
aiRouter.post('/parse-query', validateRequest(parseQuerySchema), async (req, res) => {
  try {
    const { query, context } = req.body;

    logger.info('AI query parsing request', { query, hasContext: !!context });

    const parsedQuery = await aiSearchService.parseNaturalLanguageQuery(query, context);
    ...
    res.status(500).json({
      error: 'AI Error',
      message: 'Failed to parse search query. Please try rephrasing your request.',
    });
  }
});

// GET /api/ai/suggestions - Generate search suggestions
aiRouter.get('/suggestions', validateRequest(generateSuggestionsSchema, 'query'), async (req, res) => {
  try {
    ...
    const suggestions = await aiSearchService.generateSearchSuggestions(query, context);
    ...
    logger.error('AI suggestions generation failed', { error, query: req.query.query });
...
aiRouter.post('/analyze-intent', validateRequest(analyzeIntentSchema), async (req, res) => {
  try {
    ...
    logger.error('AI intent analysis failed', { error, query: req.body.query });
    res.status(500).json({
      error: 'AI Error',
...
aiRouter.post('/recommendations', validateRequest(generateRecommendationsSchema), async (req, res) => {
  try {
    const { userData } = req.body;

    const recommendations = await aiSearchService.generatePersonalizedRecommendations(userData);
...
aiRouter.post('/enhance-descriptions', async (req, res) => {
  try {
    ...
    const enhancedHotels = await aiSearchService.enhanceHotelDescriptions(hotels);
...
aiRouter.post('/feedback', async (req, res) => {
  try {
    const { queryId, rating, feedback, type } = req.body;
    ...
    const feedbackSchema = z.object({
      ...
      type: z.enum(['query_parsing', 'suggestions', 'recommendations', 'descriptions']),
    });
