...
usersRouter.get('/profile', async (req, res) => {
  try {
    ...
    const [recentBookings] = await db
      ...
      .from(bookings)
      .where(eq(bookings.userId, userId))
      .where(gte(bookings.createdAt, new Date(Date.now() - 365 * 24 * 60 * 60 * 1000))); // Last year
...
usersRouter.put('/profile', validateRequest(updateProfileSchema), async (req, res) => {
  try {
    const userId = req.user?.id;
...
usersRouter.put('/preferences', validateRequest(updatePreferencesSchema), async (req, res) => {
  try {
    const userId = req.user?.id;
    ...
    logger.info('User preferences updated', { userId, preferences: Object.keys(newPreferences) });
    ...
    logger.error('Failed to update user preferences', { error, userId: req.user?.id });
    res.status(500).json({
...
usersRouter.get('/dashboard', async (req, res) => {
  try {
    ...
    logger.error('Failed to get user dashboard', { error, userId: req.user?.id });
    res.status(500).json({
