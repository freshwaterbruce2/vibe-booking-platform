...
async function automateIONOSRestart() {
  ...
  try {
    ...
    try {
      ...
      const performanceSelector = await page.$(
        '[data-testid*="performance"], .performance, :text("Performance Level")',
      );
      ...
      if (performanceSelector) {
        ...
        if (manageBtn) {
          ...
          const restartFound = await page.$(
            'button:contains("Restart"), button:contains("Reload"), button:contains("Reset")',
          );
          if (restartFound) {
