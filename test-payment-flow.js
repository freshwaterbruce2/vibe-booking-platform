...
async function testPaymentFlow() {
  ...
  try {
    ...
    await page
      .locator('input[placeholder*="city"], input[placeholder*="destination"]')
      .first()
      .fill('New York');
    ...
    const hotelCards = page.locator(
      '.hotel-card, [data-testid="hotel-card"], .result-item, .card, .hotel-item, [class*="hotel"]',
    );
    ...
    const roomButtons = page.locator(
      'button:has-text("Select Room"), button:has-text("Book This Room"), button:has-text("Reserve")',
    );
    ...
    const guestFormInputs = page.locator(
      'input[name*="first"], input[placeholder*="First"], input[name*="guest"]',
    );
    if ((await guestFormInputs.count()) > 0) {
      ...
      await page.screenshot({
        path: 'screenshots/payment-04-guest-info-filled.png',
        fullPage: true,
      });
    }
      ...
      try {
            ...
            if (await element.isVisible()) {
              if (selector.includes('iframe')) {
                ...
                const cardInput = frame
                  .locator('input[placeholder*="Card"], input[name*="card"]')
                  .first();
        ...
        if (cardFilled) {
          ...
          const expirySelectors = [
            ...
            'input[data-field="expirationDate"]',
          ];
          ...
          await page.screenshot({
            path: 'screenshots/payment-06-card-details-filled.png',
            fullPage: true,
          });
          ...
          for (const selector of payButtons) {
            ...
            if (await button.isVisible()) {
              ...
              payButtonFound = true;
              await page.screenshot({
                path: 'screenshots/payment-07-ready-to-pay.png',
                fullPage: true,
              });
              break;
            }
    ...
    console.log(
      `JavaScript Errors: ${errors.length === 0 ? '✅ NONE' : `❌ ${errors.length} errors`}`,
    );
    console.log(
      `Revenue Ready: ${squareFound ? '🎉 YES - CAN COLLECT MONEY!' : '❌ NO - REVENUE BLOCKED'}`,
    );
    console.log('================================\n');
  } catch (error) {
