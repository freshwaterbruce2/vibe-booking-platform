...
async function testSquareIntegration() {
  ...
  try {
    ...
    const testUrls = ['/booking', '/payment', '/checkout', '/book', '/reserve'];
      ...
      try {
        ...
        if (response && response.status() === 200) {
          ...
          await page.screenshot({
            path: `screenshots/square-test-${url.replace('/', '')}.png`,
            fullPage: true,
          });
    ...
    const hasSquareIntegration =
      finalCheck.squareScripts.length > 0 ||
    ...
    console.log(
      `💰 REVENUE CAPABILITY: ${hasSquareIntegration ? '🎉 READY TO COLLECT PAYMENTS!' : '❌ PAYMENT INTEGRATION NEEDED'}`,
    );
    console.log('=====================================\n');
