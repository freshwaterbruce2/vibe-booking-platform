    ...
    try {
      ...
      if (response.data.success && hotelsData && hotelsData.length > 0) {
        ...
        const hotels: Hotel[] = hotelsData.map((hotel: any, index: number) => ({
          ...
          amenities: (hotel.amenities || ['WiFi', 'Parking']).map((name: string, idx: number): Amenity => ({
            id: `amenity-${idx}`,
            name,
            category: 'general',
            icon: undefined,
          ...
          passionScore: hotel.passions ?
            hotel.passions.reduce((acc: Record<string, number>, p: string) => {
      ...
      const fallbackHotels: Hotel[] = [
        {
          ...
          description: 'Luxury hotel in the heart of downtown with world-class amenities and exceptional service',
          location: {
        ...
        },
        {
          ...
          description: 'Comfortable and affordable hotel perfect for business and leisure travelers',
          location: {
