...
const HotelItem = memo<HotelItemProps>(({ index, style, data }) => {
  ...
  const handleSaveHotel = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    logger.debug('Hotel saved', { component: 'VirtualizedHotelList', hotelId: hotel.id });
    // Implement save functionality
  }, [hotel.id]);

  const handleShareHotel = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    logger.debug('Hotel shared', { component: 'VirtualizedHotelList', hotelId: hotel.id });
      ...
      <Card className="overflow-hidden hover:shadow-luxury-lg transition-all duration-300 cursor-pointer">
        <div className="flex flex-col lg:flex-row h-full">
          ...
          <div className="lg:w-1/3 relative group">
            <OptimizedImage
              src={hotel.images[0]?.url || 'https://images.unsplash.com/photo-1566073771259-6a8506099945?w=400&h=300'}
              alt={hotel.name}
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-1 mb-2">
                    ...
                    <span className="text-sm text-luxury-mocha">
                      {hotel.location.neighborhood && `${hotel.location.neighborhood}, `}
                      {hotel.location.city}
                    </span>
                  </div>
                </div>
            ...
            <div className="flex items-end justify-between">
              <div className="flex flex-col">
                ...
                <div className="text-sm text-gray-500">
                  per night • {selectedDateRange.checkIn} - {selectedDateRange.checkOut}
                </div>
                <div className="text-xs text-luxury-mocha">
                  {guestCount.adults} adults{guestCount.children > 0 && `, ${guestCount.children} children`}
                </div>
              </div>
