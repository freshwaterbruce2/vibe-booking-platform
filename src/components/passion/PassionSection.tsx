...
import { Utensils, Mountain, Building2, Sparkles, Briefcase, Users2, Heart } from 'lucide-react';
...
const PassionSection: React.FC<PassionSectionProps> = ({
  ...
  const passions: Passion[] = [
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1517021897933-0e0319cfbc28?w=800&h=600&fit=crop',
    },
    {
      id: 'business-elite',
      name: 'Business Elite',
    ...
    },
    {
      ...
      image: 'https://images.unsplash.com/photo-1502301103665-0b95cc738daf?w=800&h=600&fit=crop',
    },
  ];
  ...
  return (
    <section className={`relative bg-gradient-to-br from-slate-50 via-white to-amber-50/30 py-20 overflow-hidden ${className}`}>
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {passions.map((passion) => {
            ...
            return (
              ...
              >
                ...
                {/* Gradient Overlay */}
                <div className={`absolute inset-0 bg-gradient-to-b from-black/40 via-black/50 to-black/70 ${isSelected ? 'opacity-90' : 'opacity-80'} transition-opacity duration-500 group-hover:opacity-70`} />
                
                {/* Color Gradient for Selected State */}
                <div className={`absolute inset-0 bg-gradient-to-br ${passion.color} ${isSelected ? 'opacity-40' : 'opacity-0'} transition-opacity duration-500`} />
                ...
                <div className="relative p-6 h-full flex flex-col">
                  ...
                  <div className="text-center">
                    {/* Professional Icon */}
                    <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center transition-all duration-500 backdrop-blur-md ${
                      isSelected 
            ...
            <div className="text-center">
              <div className="flex items-center justify-center gap-3 mb-4">
                <div className={`w-3 h-3 rounded-full transition-colors duration-300 ${
                  localSelectedPassions.length > 0 ? 'bg-slate-600' : 'bg-slate-300'
                }`} />
                <p className="text-xl font-semibold text-slate-800">
                  <span className="text-2xl text-slate-700 font-bold">{localSelectedPassions.length}</span> passion{localSelectedPassions.length !== 1 ? 's' : ''} selected
                </p>
                <div className={`w-3 h-3 rounded-full transition-colors duration-300 ${
                  localSelectedPassions.length > 0 ? 'bg-slate-600' : 'bg-slate-300'
                }`} />
        ...
        {localSelectedPassions.length > 0 && (
          <div className="mt-12 p-8 bg-white/80 backdrop-blur-sm rounded-2xl shadow-luxury-lg border border-slate-200/50">
            <h4 className="text-xl font-bold text-slate-900 mb-4 text-center">Your Luxury Travel Preferences</h4>
            <div className="flex flex-wrap gap-3 justify-center">
              {localSelectedPassions.map((passionId) => {
                ...
                const IconComponent = passion ? {
                  ...
                  users2: Users2,
                  heart: Heart,
                }[passion.icon] || Utensils : Utensils;
