...
describe('SearchStore', () => {
  ...
  describe('Query Actions', () => {
    ...
    it('should set natural language query correctly', () => {
      ...
      setNaturalLanguageQuery('Find me a luxury hotel in Paris for next weekend');
      
      const state = useSearchStore.getState();
      expect(state.naturalLanguageQuery).toBe('Find me a luxury hotel in Paris for next weekend');
    });

    it('should clear search and reset relevant state', () => {
      const { setQuery, setResults, setError, setPagination, setAiProcessedQuery, clearSearch } = useSearchStore.getState();
  ...
  describe('Complex Scenarios', () => {
    it('should handle full search workflow', () => {
      ...
      // Start search
      store.setNaturalLanguageQuery('Luxury hotel in Paris for 2 adults, December 1-3');
      ...
      const state = useSearchStore.getState();
      expect(state.naturalLanguageQuery).toBe('Luxury hotel in Paris for 2 adults, December 1-3');
      expect(state.loading).toBe(false);
      expect(state.results).toEqual(testUtils.mockSearchResults);
