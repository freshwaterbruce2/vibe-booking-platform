...
describe('UserStore', () => {
  ...
  describe('Passion Management', () => {
    it('should set passion data', () => {
      ...
      const state = useUserStore.getState();
      expect(state.passions.selectedPassions).toEqual(['romantic-escape', 'luxury-indulgence']);
    });
  ...
  describe('Saved Hotels Management', () => {
    ...
    it('should handle multiple saved hotels', () => {
      ...
      const state = useUserStore.getState();
      expect(state.savedHotels).toEqual(['hotel-123', 'hotel-456', 'hotel-789']);
    });
  });
  ...
  describe('Complex User Scenarios', () => {
    ...
    it('should handle complete user journey with passions and history', () => {
      ...
      store.setPassions({
        selectedPassions: ['romantic-escape', 'luxury-indulgence', 'wellness-retreat'],
      });
      ...
      expect(finalState.bookingHistory[0].hotelName).toBe('Tuscany Wellness Spa Resort');
    });
  });
});