...
import type { Hotel, Room, Booking, SearchFilters, PassionScore } from '@/types/api';
import type { PaymentInfo, GuestDetails } from '@/store/bookingStore';
...
export class MockDataGenerator {
  ...
  static generateHotel(overrides: Partial<Hotel> = {}): Hotel {
    const baseId = overrides.id || `hotel-${Math.random().toString(36).substr(2, 9)}`;
    const starRating = overrides.rating || (Math.random() * 2 + 3); // 3-5 stars

    return {
      id: baseId,
      name: overrides.name || `Hotel ${baseId.slice(-5)}`,
      description: overrides.description || 'A wonderful hotel with great amenities and excellent service.',
      location: {
      ...
      availability: {
        ...
        priceChange: Math.random() < 0.5 ? undefined : Math.floor((Math.random() - 0.5) * 50),
        ...overrides.availability,
      },
      passionScore: this.generatePassionScore(overrides.passionScore),
  ...
  static generatePassionScore(overrides: Partial<PassionScore> = {}): PassionScore {
    const passions = [
      'luxury-indulgence', 'wellness-retreat', 'romantic-escape', 'adventure-seeker',
      'cultural-explorer', 'budget-conscious', 'family-fun', 'business-traveler',
      'eco-conscious', 'foodie-experience'
    ] as const;
    ...
    passions.forEach(passion => {
      scores[passion] = overrides[passion] ?? (Math.random() < 0.3 ? Math.random() * 0.7 + 0.3 : Math.random() * 0.3);
    });
  ...
  static generateFamilyHotel(): Hotel {
    return this.generateHotel({
      ...
      amenities: [
        ...
        { id: 'restaurant', name: 'Family Restaurant', category: 'dining', icon: '🍽️' },
        { id: 'babysitting', name: 'Babysitting', category: 'service', icon: '👶' },
      ],
      passionScore: {
        'family-fun': 0.95,
  ...
  static generateRoom(overrides: Partial<Room> = {}): Room {
    ...
    return {
      ...
      type: overrides.type || roomTypes[Math.floor(Math.random() * roomTypes.length)] as any,
      ...
      images: overrides.images || [`room-${Math.random().toString(36).substr(2, 5)}.jpg`],
      availability: overrides.availability ?? true,
      description: overrides.description || 'A comfortable room with modern amenities.',
  ...
  static generateBooking(overrides: Partial<Booking> = {}): Booking {
    ...
    return {
      ...
      totalAmount: overrides.totalAmount || Math.floor(Math.random() * 1000) + 200,
      currency: overrides.currency || 'USD',
      status: overrides.status || 'confirmed',
      paymentStatus: overrides.paymentStatus || 'completed',
      createdAt: overrides.createdAt || new Date().toISOString(),
      guestDetails: overrides.guestDetails || this.generateGuestDetails(),
      ...overrides,
    };
  }

  // Generate mock guest details
  static generateGuestDetails(overrides: Partial<GuestDetails> = {}): GuestDetails {
    const firstNames = ['John', 'Jane', 'Michael', 'Sarah', 'David', 'Emma', 'Chris', 'Lisa'];
    const lastNames = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis'];

    return {
      firstName: overrides.firstName || firstNames[Math.floor(Math.random() * firstNames.length)],
      lastName: overrides.lastName || lastNames[Math.floor(Math.random() * lastNames.length)],
      email: overrides.email || `user${Math.random().toString(36).substr(2, 5)}@example.com`,
      phone: overrides.phone || `+1${Math.floor(Math.random() * 9000000000) + 1000000000}`,
  ...
  static generatePaymentInfo(overrides: Partial<PaymentInfo> = {}): PaymentInfo {
    return {
  ...
  static generateSearchFilters(overrides: Partial<SearchFilters> = {}): SearchFilters {
    return {
  ...
  static generateEdgeCaseHotels() {
    return {
      ...
      maximal: this.generateHotel({
        name: 'Maximal Luxury Resort & Spa Experience',
        description: 'An extraordinary luxury resort with world-class amenities, exceptional service, and breathtaking views that create unforgettable memories for our distinguished guests.',
        amenities: [
          { id: 'wifi', name: 'WiFi', category: 'connectivity', icon: '📶' },
          { id: 'spa', name: 'Spa', category: 'wellness', icon: '🧖' },
          { id: 'pool', name: 'Pool', category: 'recreation', icon: '🏊' },
          { id: 'gym', name: 'Fitness Center', category: 'fitness', icon: '💪' },
          { id: 'restaurant', name: 'Fine Dining', category: 'dining', icon: '🍽️' },
          { id: 'bar', name: 'Cocktail Bar', category: 'dining', icon: '🍸' },
          { id: 'concierge', name: 'Concierge', category: 'service', icon: '🛎️' },
          { id: 'valet', name: 'Valet Parking', category: 'service', icon: '🚗' },
          { id: 'business', name: 'Business Center', category: 'business', icon: '💼' },
          { id: 'meeting', name: 'Meeting Rooms', category: 'business', icon: '🏢' },
        ],
        rating: 4.9,
        reviewCount: 5000,
      ...
      perfectRating: this.generateHotel({
      ...
      specialChars: this.generateHotel({
  ...
  static generateLargeHotelDataset(count: number = 100): Hotel[] {
  ...
  static generateErrorScenarios() {
    return {
      ...
      longStrings: this.generateHotel({
        name: 'A'.repeat(500),
        description: 'B'.repeat(2000),
      }),

      // Hotel with malformed coordinates
      invalidCoordinates: this.generateHotel({
        location: {
          address: '123 Test St',
          city: 'Test City',
          country: 'TC',
...
export const testHelpers = {
  ...
  generateFutureDate: (daysFromNow: number = 30) => {
    const date = new Date();
  ...
  simulateAsyncDelay: (ms: number = 100) => new Promise(resolve => setTimeout(resolve, ms)),
