...
describe('BookingFlow Component', () => {
  ...
  describe('Step Indicator', () => {
    ...
    it('highlights current step correctly', () => {
      ...
      const stepIndicators = screen.getAllByRole('generic').filter(el => 
        el.className.includes('border-primary-600')
      );
  ...
  describe('Room Selection Step', () => {
    it('displays selected room information correctly', () => {
      ...
      expect(screen.getByText('A spacious room with modern amenities')).toBeInTheDocument();
    ...
    it('shows no room selected message when room is not available', () => {
      ...
      expect(screen.getByText('No room selected. Please select a room first.')).toBeInTheDocument();
    });
  });
  ...
  describe('Guest Details Step', () => {
    ...
    it('renders guest details form fields', () => {
      ...
      expect(screen.getByPlaceholderText('Any special requests or preferences?')).toBeInTheDocument();
    });
    ...
    it('calls setGuestDetails when form fields are updated', async () => {
      ...
      expect(mockBookingStore.setGuestDetails).toHaveBeenCalledWith({ firstName: 'John' });
    });
  ...
  describe('Confirmation Step', () => {
    ...
    it('displays terms and conditions', () => {
      ...
      expect(screen.getByText(/By completing this booking/)).toBeInTheDocument();
    });
  });
