...
describe('CancellationForm Component', () => {
  ...
  describe('Cancellable Booking Flow', () => {
    ...
    it('shows free cancellation status when within deadline', () => {
      ...
      expect(screen.getByText(/You have \d+ hours remaining/)).toBeInTheDocument();
    });
    ...
    it('shows cancellation policy details', () => {
      ...
      expect(screen.getByText('50% refund for cancellations 12-24 hours before check-in')).toBeInTheDocument();
      expect(screen.getByText('No refund for same-day cancellations')).toBeInTheDocument();
    });
  ...
  describe('Confirmation Dialog', () => {
    ...
    it('requires cancellation reason before allowing confirmation', async () => {
      ...
      const reasonTextarea = screen.getByPlaceholderText('Please provide a reason for cancellation...');
    ...
    it('shows refund processing information', () => {
      expect(screen.getByText('Refund Processing Information:')).toBeInTheDocument();
      expect(screen.getByText('Refunds typically take 5-10 business days to appear on your statement')).toBeInTheDocument();
      expect(screen.getByText('You will receive an email confirmation once the refund is processed')).toBeInTheDocument();
    });
    ...
    it('processes cancellation with reason and refund amount', async () => {
      ...
      expect(mockProps.onCancel).toHaveBeenCalledWith('Emergency situation', 300);
    });
  ...
  describe('Non-Cancellable Booking', () => {
    ...
    it('shows non-cancellable message for non-refundable bookings', () => {
      ...
      expect(screen.getByText('Cancellation Not Available')).toBeInTheDocument();
      expect(screen.getByText('This booking has a non-refundable policy and cannot be cancelled.')).toBeInTheDocument();
    });

    it('provides customer support contact option', () => {
      render(<CancellationForm {...nonCancellableProps} />);
      
      expect(screen.getByText('For special circumstances, please contact our customer support team who may be able to assist you.')).toBeInTheDocument();
      
      const supportLink = screen.getByText('Contact Support');
      expect(supportLink).toHaveAttribute('href', 'mailto:support@hotelbooking.com');
    });
  ...
  describe('Past Deadline Booking', () => {
    ...
    it('shows deadline passed message', () => {
      ...
      expect(screen.getByText('Cancellation Not Available')).toBeInTheDocument();
      expect(screen.getByText('The cancellation deadline has passed.')).toBeInTheDocument();
  ...
  describe('Accessibility', () => {
    ...
    it('has proper form labels and ARIA attributes', () => {
      ...
      const reasonSelect = screen.getByLabelText('Reason for Cancellation (Optional)');
      expect(reasonSelect).toBeInTheDocument();
