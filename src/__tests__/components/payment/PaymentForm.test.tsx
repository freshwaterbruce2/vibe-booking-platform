...
vi.mock('@/components/payment/PaymentSummary', () => ({
  PaymentSummary: ({ amount, currency, bookingDetails }: any) => (
    <div data-testid="payment-summary">
      ...
      {bookingDetails && (
        <div>Hotel: {bookingDetails.hotelName}</div>
      )}
    </div>
  ),
}));
...
describe('PaymentForm', () => {
  ...
  describe('Loading State', () => {
    it('should show loading spinner initially', async () => {
      ...
      vi.mocked(require('@/services/payment')).stripePromise = delayedStripePromise;
  ...
  describe('Error Handling', () => {
    it('should show error when Stripe fails to load', async () => {
      const failedStripePromise = Promise.reject(new Error('Failed to load Stripe'));
      vi.mocked(require('@/services/payment')).stripePromise = failedStripePromise;
    ...
    it('should show error when Stripe is not properly configured', async () => {
      ...
      vi.mocked(require('@/services/payment')).stripePromise = nullStripePromise;

      render(<PaymentForm {...defaultProps} />);

      await waitFor(() => {
        expect(screen.getByText('Payment System Error')).toBeInTheDocument();
        expect(screen.getByText('Payment system is not properly configured')).toBeInTheDocument();
      });
    });
  ...
  describe('Successful Payment Flow', () => {
    ...
    it('should call onSuccess when payment succeeds', async () => {
      ...
      expect(defaultProps.onSuccess).toHaveBeenCalledWith(testUtils.mockPaymentIntent);
    });
  });
  ...
  describe('UI Elements and Layout', () => {
    ...
    it('should display secure payment header', () => {
      ...
      expect(screen.getByText('Your payment is protected with industry-standard encryption')).toBeInTheDocument();
    });
  ...
  describe('Props Handling', () => {
    ...
    it('should work without optional billingDetails', async () => {
      ...
      expect(() => render(<PaymentForm {...propsWithoutBillingDetails} />)).not.toThrow();
    });
  ...
  describe('Accessibility', () => {
    ...
    it('should have proper heading hierarchy', () => {
      expect(screen.getByRole('heading', { level: 2, name: /secure payment/i })).toBeInTheDocument();
      expect(screen.getByRole('heading', { level: 3, name: /payment details/i })).toBeInTheDocument();
    });
    ...
    it('should provide clear error messages', async () => {
      ...
      await waitFor(() => {
        ...
        expect(errorElement).toHaveAttribute('class', expect.stringContaining('font-medium'));
      });
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle invalid booking ID', async () => {
      ...
      expect(() => render(<PaymentForm {...invalidBookingProps} />)).not.toThrow();
