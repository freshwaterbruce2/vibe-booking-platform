...
vi.mock('@/services/payment', () => ({
  PaymentService: {
    ...
    formatCurrency: vi.fn((amount, currency) => `${currency}$${(amount / 100).toFixed(2)}`),
  },
}));
...
describe('PaymentElementForm', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(PaymentService.createPaymentIntent).mockResolvedValue(mockPaymentIntent);
  ...
  describe('Initialization', () => {
    it('should show loading state during payment setup', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('Setting up payment...')).not.toBeInTheDocument();
      });
    });
    ...
    it('should render payment element after successful initialization', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Billing Email: john.doe@example.com')).toBeInTheDocument();
      });
    });
  ...
  describe('Error Handling During Initialization', () => {
    it('should show error when payment intent creation fails', async () => {
      ...
      vi.mocked(PaymentService.createPaymentIntent).mockRejectedValue(new Error(errorMessage));
    ...
    it('should not render payment form when initialization fails', async () => {
      vi.mocked(PaymentService.createPaymentIntent).mockRejectedValue(new Error('Network error'));
    ...
    it('should call onError with generic message for unknown errors', async () => {
      vi.mocked(PaymentService.createPaymentIntent).mockRejectedValue('Unknown error');
  ...
  describe('Payment Errors', () => {
    ...
    it('should handle payment requiring additional authentication', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Additional authentication required. Please complete the verification.')).toBeInTheDocument();
      });
    ...
    it('should handle incomplete payments', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Payment was not completed successfully')).toBeInTheDocument();
      });

      expect(defaultProps.onError).toHaveBeenCalledWith('Payment was not completed successfully');
    });
  });
  ...
  describe('Cleanup and Memory Management', () => {
    it('should handle component unmounting during initialization', async () => {
      ...
      vi.mocked(PaymentService.createPaymentIntent).mockReturnValue(paymentPromise);
  ...
  describe('Accessibility', () => {
    ...
    it('should have security notice for user confidence', () => {
      ...
      expect(screen.getByText(/We do not store your card details/)).toBeInTheDocument();
    });
  });
});