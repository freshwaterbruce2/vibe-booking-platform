...
describe('HotelDetails Component', () => {
  ...
  describe('Rendering', () => {
    it('should render hotel details when provided', () => {
      ...
      expect(screen.getByText('A beautiful luxury hotel with amazing amenities')).toBeInTheDocument();
      expect(screen.getByText('123 Luxury Avenue, Test City')).toBeInTheDocument();
  ...
  describe('Navigation', () => {
    it('should handle back to results action', () => {
      ...
      render(
        <TestWrapper>
          <HotelDetails hotel={mockHotel} onBackToResults={mockOnBackToResults} />
        </TestWrapper>
      );
  ...
  describe('Error Handling', () => {
    ...
    it('should handle API errors during booking', async () => {
      const mockOnBookRoom = vi.fn().mockRejectedValue(new Error('Booking failed'));
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle extreme price ranges', () => {
      const hotelWithExtremePrices = MockDataGenerator.generateHotel({
        ...mockHotel,
        priceRange: { min: 50000, max: 100000, avgNightly: 75000, currency: 'USD' },
      }) as HotelDetailsType;
