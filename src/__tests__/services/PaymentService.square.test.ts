...
describe('PaymentService (Square)', () => {
  ...
  it('creates a Square payment successfully', async () => {
    mockedAxios.post.mockResolvedValueOnce({ data: { success: true, paymentId: 'sq_pay_123', receiptUrl: 'https://receipt' } });
    const result = await PaymentService.createSquarePayment({ bookingId: 'b1', amount: 100, sourceId: 'tok_123', currency: 'USD' });
    expect(result.success).toBe(true);
    expect(result.paymentId).toBe('sq_pay_123');
  });

  it('handles Square payment validation error', async () => {
    await expect(PaymentService.createSquarePayment({ bookingId: 'b1', amount: -10, sourceId: 'tok_123', currency: 'USD' } as any)).rejects.toThrow();
  });
  ...
  it('creates refund', async () => {
    mockedAxios.post.mockResolvedValueOnce({ data: { success: true, refundId: 'rf_123' } });
    const res = await PaymentService.createRefund({ paymentId: 'sq_pay_123', bookingId: 'b1', amount: 50, reason: 'test' } as any);
